...
- 状态：`reviewed`，修复追踪已与 `c912c48` 对齐
- 审计基线：`98ea37f1bc298fb6c718a037127b487af01b0888`
- 修复追踪基线：`c912c4851c763b826bcf7d826a56b50f9ee669f9`
- 审计日期：2026-07-24
...
但它**尚不足以安全承载生产部署或扩大协作范围**。本次静态审计在原始基线确认 3 项
P0：
...
截至修复追踪基线，第一项已由 PR #197 闭合；上传解析前硬上限和 `0034` N-1
迁移证据也分别由 PR #198、#199 闭合。过期 owner、Learner Profile 来源授权、匿名
对象文件与渠道绑定生命周期仍未闭合，应在 Memory 或家庭/班级协作继续扩面前处理。

本报告没有修改业务代码、Schema、migration journal、Memory 或 UI。

### 1.1 审计后修复追踪

下表记录审计完成后的独立原子修复。它不改写审计基线；“已合并”只表示对应 PR 在最新
追踪时远程 CI 全绿后进入 `main`。

| 风险/质量项                            | PR   | main 提交 | 当前状态 | 主要证据                                                                                   |
| -------------------------------------- | ---- | --------- | -------- | ------------------------------------------------------------------------------------------ |
| A-01 Operation/Approval 撤权后仍可访问 | #197 | `1cd8d13` | 已闭合   | 当前 Membership 复核、conversation/notebook 绑定、事务锁和真实 PostgreSQL/HTTP 撤权负例    |
| A-04 multipart 先物化后限流            | #198 | `97a4592` | 已闭合   | 复用共享 bounded multipart 读取器；声明长度、真实流、畸形 multipart 和稳定错误码测试       |
| A-07 `0034` migration 证据漂移         | #199 | `ed21387` | 已闭合   | 独立 `0033` 与 `0034` Drizzle bundle，验证 additive 升级、DDL/journal 回滚与前向重跑       |
| 部署 env guard 负例不足                | #200 | `c912c48` | 已增强   | HTTPS、provider/runtime、URL 凭据、密钥/模型格式和数值边界负例，远程 Linux/Windows CI 全绿 |

A-02/A-03 会改变 Learner Profile/Goal 权威语义；A-05/A-06 需要对象删除 outbox、
匿名 identity/channel 最小保留或 FK 决策。它们仍只保留方案，必须通过独立 ADR/PR
推进，不能从上述已闭合项外推为已经解决。
...
审计开始时已执行远程引用拉取；当时 test worktree 的 HEAD 与 `origin/main` 均为
`98ea37f1bc298fb6c718a037127b487af01b0888`。该提交继续作为发现问题的不可变审计
基线；后续修复状态单独以本文件头部的修复追踪基线为准。
...
`docs/04-data/01-学习计划与诊断.md:13`、
`docs/09-decisions/0007-学习者画像与学习计划可信边界.md:17-23`。但
...
- 备份、PITR、restore 后重放删除清单和“被删除匿名数据不复活”仍是文档中的生产门，
  ...
  `docs/04-data/02-数据设计.md:220-228`；
...
`tooling/architecture/runtime-module-size-boundary.test.mjs:102-253` 只检查选定文件，没有覆盖这些模块。
...
下表保留 `98ea37f` 上的原始发现，避免修复后抹去审计历史。截至 `c912c48`，A-01、
A-04、A-07 已分别由 PR #197、#198、#199 闭合；其余条目仍是待处理风险。
...
| A-03 | P0   | Learner Profile 的 guardian/school provenance 可由调用者自报         | `study-plan-repository.ts:177-221,232-282`；ADR-0007                               |
...
1. `[已合并 #197] fix/20260724-operation-access-revalidation`
...
3. `fix/20260724-profile-provenance`
   ...
   - 若 contract 需变化，先补 ADR-0007 amendment。
4. `[已合并 #198] fix/20260724-upload-body-limit`
   - parser 前声明长度/真实流 hard limit + 超限和畸形 multipart 测试。
5. `[已合并 #199] test/20260724-migration-0034-compatibility`
...
13. `[已合并 #200] test/20260724-env-check-boundaries`
