...
- 状态：`accepted`
- 负责人：项目负责人
- 最后验证时间：2026-08-12（C01-C07 自动化收口）
- 关键决策：[ADR-0004](../09-decisions/0004-能力信任与学习证据.md)、[ADR-0005](../09-decisions/0005-模块化单体产物与持久任务.md)、[ADR-0009](../09-decisions/0009-统一画布工作面与运行时分层.md)、[ADR-0026](../09-decisions/0026-多模态输入原件与派生表示边界.md)、[ADR-0027](../09-decisions/0027-Canvas多形态输出与交互运行时边界.md)
...
PDF、图片、网页、文本、音频和视频仍是 Source 事实。Canvas 通过原生 Renderer 打开原件，并可切换到已鉴权的结构化 Markdown、派生图片或转录 Representation；派生表示不能冒充或改写原始来源。标注或摘录形成单独的用户事实或派生 Artifact。

### 3.2 结构化 Artifact

Markdown 文档、Slides、`mind_map.v1/v2`、闪卡、笔记、练习和参数化动画使用有界 Schema 与预注册 React Renderer。内容版本不可变，修改产生新版本；Renderer 只解释协议，不拥有 Notebook 权限或生成任务。未知 kind/contentVersion/renderer 必须诚实失败，历史 v1 不因 v2 上线静默失效。
...
`web_app.v1` 的 HTML/JavaScript 与平台审计的依赖包在隔离沙箱运行。模型不能注入主页面，也不能动态从任意 CDN 或 npm 下载依赖。允许的依赖固定版本并由平台提供；沙箱默认禁同源、禁网络、禁 Credential，桥接消息使用版本化白名单协议。

隔离由 `apps/web-runtime` 这个独立进程承载：它必须与 Web 处于**不同的 schemeful site**，否则 `readWebRuntimeConfig` 与 Web 侧的 `readWebRuntimeHostConfig` 都会 fail closed——边界不成立就拒绝启动，而不是降级成同源。端口不参与 site 判定，仅换端口不构成边界。本地默认站点对是 Web `http://127.0.0.1:3000` 与 Runtime `http://localhost:3300`（`localhost` 与 `127.0.0.1` 属不同站点），因此无需 hosts 文件；`make all` 把它作为 core 服务之一拉起，`make status` 以 Web Runtime 行呈现。
...
- General Agent 在用户选择 `auto`、`markdown_document`、`interactive_artifact` 或 `web_app` 时仍通过同一个受控 Artifact Tool 发起闭集 Proposal；输出偏好不改变 Tool grant；
- Markdown 文档编辑/导出、`mind_map.v2` 确定性布局与交互、`web_app.v1` 构建/诊断/取消/隔离运行以及三类 Artifact 的不可变版本、回退和继续修改已完成 C01-C07 自动化门禁；真实生成和大图体验仍由 C08 真人验收确认；
...
- 扩展 Web App 依赖白名单与发布治理；当前 `web_app.v1` 保持无网络、无运行时安装的闭集边界；
