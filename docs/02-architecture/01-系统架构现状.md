...
- 关键决策：[ADR-0001](../09-decisions/0001-education-centered-personal-agent-platform.md)、[ADR-0002](../09-decisions/0002-gateway-clients-channels-and-nodes.md)、[ADR-0003](../09-decisions/0003-unified-runtime-and-notebook-context.md)、[ADR-0005](../09-decisions/0005-modular-monolith-artifacts-and-durable-jobs.md)
...
Canvas 当前是 Web 中的 Artifact 工作面，不是 Notebook 的事实源。Source、Artifact 和运行记录仍由各自仓储持有；下一阶段已接受的统一工作面边界见[统一 Canvas 工作面](04-统一Canvas工作面.md)。

[完整架构总览（SVG）](educanvas-architecture-overview.svg)用于快速理解入口、控制面、Agent Runtime、可信领域服务与持久任务之间的关系；本页文字和已接受 ADR 仍是可检索的事实依据。
...
当前 Web 已能渲染结构化判分 Artifact、参数化动画、思维导图、Slides、闪卡、音频概览和笔记，并能按用户显式操作运行消息内的隔离 HTML 预览。Source 与 Artifact 尚未共享统一 `CanvasResource` 和 Renderer Registry；PDF/图片/音视频统一预览、持久沙箱应用、生图、代码执行和机器学习 Runtime 均未落地。

这些后续能力已在 [ADR-0009](../09-decisions/0009-统一Canvas工作面与运行时分层.md)确定边界，但不能写成当前实现。
