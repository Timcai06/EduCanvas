...
| `packages/agent-runtime`                      | 唯一 `AgentLoopEngine`、模型流验证、预算、取消和通用工具注册基础               | 不拥有 Notebook 归属或 K12 事实              |
...
当前只有一个生产模型/工具循环 `AgentLoopEngine`，但有三条调用它的应用路径：

1. `apps/gateway/src/agent-runner.ts`：TUI/Channel 等 Gateway 入口；当前注入的工具集合为空或受限；
2. `apps/web/server/platform/general-turn.ts`：Web 通用 Chat，负责自己的 Context、消息账本与工具组合；
3. `apps/web/server/teaching/learning-turn.ts` 经 `packages/teaching-runtime/src/turn-orchestrator.ts`：Web 教学路径，额外处理安全、课程状态、判分、学习事件和教学工具。
...
- Gateway Operation Event 是跨入口控制面事实；Web 消息/Turn 账本继续服务产品兼容；
- `model_runs` 目前仍带教学 Turn 形状约束，不能称为通用模型审计；
