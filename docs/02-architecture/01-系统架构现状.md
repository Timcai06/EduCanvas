...
Web 浏览器传输仍使用 `/api/v1/chat/*` 与 `/api/v1/learn/*` 的 BFF/SSE 兼容层；TUI、Channel 和 Node 通过独立 Gateway 进程。两条浏览器协议没有被强制改成 Gateway 的持久 NDJSON 协议，这是 ADR-0016 的兼容选择。Gateway、Web General 与 Web Teaching 的服务端执行语义已经收敛到同一个 Turn Application；Node 的 L0/L1 只读 Adapter 已在 Gateway 与 Web General 接入，Asset 与 MCP 等 Adapter 仍在后续阶段。
...
| `apps/gateway`                                | HTTP 组合根、Client/Node session、目录、审批、NDJSON、Node Tool 与内部指标                   | Asset 与 MCP 等 Adapter 尚未接入             |
...
| `packages/teaching-runtime`                   | K12 Prompt、安全、课程 Workflow、领域回调与 Teaching Tool Adapter                            | 遗留 Orchestrator 仅待无调用清理             |
...
当前三条生产 Turn 调用路径都进入 `TurnApplicationService`；`packages/teaching-runtime` 中遗留且无生产调用者的旧 Orchestrator 待清理阶段物理删除。生产应用路径现为：

1. `apps/gateway/src/agent-runner.ts`：把可信路由投影到`TurnApplicationService`并投影事件；独立Gateway Profile已接入私人Node的L0/L1只读工具，Asset仍未接通；
...
- Web General网页工具、Web Teaching教学工具以及Gateway/Web General的Node工具都通过生产`ToolKernel`执行；`AgentToolRegistry`和`TeachingToolExecutor`已无生产调用者，待统一清理；
...
当前不具备 production 声明条件：正式 IdP、自助账号生命周期、外部 Trace/SLO、对象删除闭环、原生多模态、长期学习者记忆、审批后continuation、跨进程lease/cancel、Telegram live凭据验证和微信/QQ Adapter仍未完成。Gateway bootstrap token是管理员或本地建联凭据，不是最终用户登录方案。
