...
| `apps/gateway`                                | HTTP组合根、Client/Node session、目录、审批、NDJSON、Node/MCP Tool与内部指标                 | Asset与高风险MCP续跑尚未接入                 |
...
- Web General网页工具、Web Teaching教学工具以及Gateway/Web General的Node和MCP工具都通过生产`ToolKernel`执行；旧通用注册表与教学执行器已删除，本地/教学模块只保留定义和Adapter；
...
剩余差异是 Profile 和入口能力差异，不再是第二套 Tool Runtime。后续迁移门见[第二代架构提案](03-第二代架构提案.md)。
