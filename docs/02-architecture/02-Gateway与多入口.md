...
| `GET /v1/client/connections`             | Provider 目录与主体连接状态   | session bearer + tenant filter   |
| `POST /v1/client/connections/connect`    | 发起外部渠道一次性授权        | session bearer + route check     |
| `POST /v1/client/connections/revoke`     | 撤销并审计主体连接            | session bearer + owner check     |
| `POST /v1/client/handoffs`               | 签发短期一次性 Web 交接凭证   | session bearer + owner check     |
...
`apps/tui` 只依赖 `gateway-client/gateway-core`。本地 loopback 首次进入时由 Gateway 为固定 local registered user 幂等准备 Personal Agent、默认 Notebook/Conversation 和短期 session，不再要求用户输入 ID 或共享 bootstrap token。TUI 提供持续交互 Chat、Notebook 切换、状态、恢复、审批、`/channels` 与 Web 入口；非 TTY 自动化仍可使用原子子命令。session 配置写入 `~/.config/educanvas/client.json`，目录/文件权限为 0700/0600。

TUI `/web` 不再把 Conversation ID 当作 URL 授权，而是经 `POST /v1/client/handoffs` 请求短期一次性凭证。Web `/open` 使用当前服务端身份消费凭证；PostgreSQL 的条件更新提供多进程下的到期、单次消费和主体隔离。

### Telegram

Telegram 私聊文本是已完成的实验性协议纵切，不是当前产品优先渠道，也不由 `make all` 默认启动。配置 Bot username 后，Web 设置页或 TUI 创建十分钟 pending 与官方 `/start` deep link；Adapter 从私聊提取 connection ID，再由 PostgreSQL 校验 pending、到期、重放和外部账号唯一性，普通 Update 不能自建绑定。激活后 Adapter 只接受非 bot、private chat 的文本 Update，以官方 `update_id` 形成幂等键，拒绝群聊、未知账号和媒体；输出按 `sendMessage` 4096 字符上限切分，不使用 `parse_mode`。Delivery 在 PostgreSQL 中去重和记录回执，offset 只在成功处理 Update 后推进。仓库仍只有官方形状 Fixture，没有 live 账号发送证据。

### Capability Node

Node 主动配对并轮询 Gateway，只开放 `device.status` 和 `filesystem.read_allowlisted`。两项能力通过`packages/node-runtime`进入Web General与Gateway/TUI共用的Tool Kernel：Profile每个Turn都以Operation真实Actor/Agent和近期心跳重新计算能力，模型与客户端不能选择`nodeId`，共享Notebook成员也不能继承所有者私人Node。调用使用稳定request/nonce幂等入队，等待期间响应取消与到期，结果再经过有上限的输出Schema；Node离线或仓储降级时工具不暴露且云端对话继续。

文件能力要求预配置 root alias，拒绝绝对路径、`..`、symlink escape、超限文件、过期/重放/撤销请求；代码和依赖边界同时禁止 `child_process`、shell 与写操作。本阶段风险仍为L0/L1，不借只读Node虚构审批后高风险续跑。
