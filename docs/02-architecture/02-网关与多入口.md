...
- 负责人：项目负责人
- 最后验证时间：2026-08-13
- 关键决策：[ADR-0002](../09-decisions/0002-网关客户端渠道与能力节点.md)、[ADR-0004](../09-decisions/0004-能力信任与学习证据.md)、[ADR-0028](../09-decisions/0028-桌宠作为统一EduCanvas系统的第一方桌面外延.md)
...
- `client`：TUI、Desktop Pet 与其他远程第一方交互应用；Web 是共享控制面语义的共进程 BFF Client Adapter；
...
ADR-0028 将 Desktop Pet 定义为统一 EduCanvas 系统的能力受限第一方桌面外延。当前
`apps/desktop` 已实现 Electron 透明角色窗口、托盘与拖动、文本问答、显式录音、整段 ASR、
TTS、系统浏览器 PKCE、可撤销桌面会话、`gateway.v1` Turn 和独立聊天窗口；它使用同一
Personal Agent 和 Agent Runtime，不复用打包环境中的 `local:owner`，也不直接接触 Provider
Secret。

当前仍未统一的部分必须准确区分：桌面 Session 仍固定保存授权时的 Notebook/Conversation；
聊天历史仍是 Electron main 内存 View Cache；Gateway 事件在 main 聚合成最终字符串后才返回
renderer；Citation、Artifact、工具状态、Operation resume 与 Web handoff 尚未进入桌面投影。
DP01-DP09 将按身份/游标解耦、canonical 历史、真实事件流、结构化结果和精确 handoff 渐进
收敛，不推倒重写现有桌宠。

Web 的 `general/restricted` 体验模式门禁只约束 Web 入口，不能替代桌面端的主 frame 麦克风
权限、用户手势、瞬时音频和服务端 Provider 边界。桌宠真实 Provider 与真人麦克风仍需独立
发布证据。
