...
`TurnApplicationService`核心也已落地为唯一目标编排点：它先固化Context Snapshot，再为每次经Runtime验证的Provider调用结算通用Model Run，所有模型工具调用进入同一Tool Kernel，最后经过Profile确定性复核写入唯一消息终态。它对Context与实际Prompt做防漂移校验，旧Operation replay不再次调用Provider，Trace exporter失败不改变业务结果。Gateway文本Turn与Web General均已迁入该服务，并由Gateway独占Operation终态；Web General同时接入结构化Asset片段、网页Tool/Effect和同事务引用结算。源码只剩Web Teaching旧编排直接构造Loop；该入口切换并通过扫描后，才满足“生产唯一构造位置”。
...
Gateway runner与Web General已经投影唯一`TurnApplicationService`；Web Teaching仍自行装配Context、Prompt、工具、消息账本与回调。入口能力仍未完全相等：Web General已有Asset与网页Tool，独立Gateway Profile尚未复用这些Adapter。后续先迁Web Teaching，再把通用Tool/Asset装配提升为Gateway与Web共享组合，不复制Loop。
...
Local AgentTool与Teaching Tool现已有兼容Adapter。通用可信上下文显式传递Operation、Conversation、Trace、Actor、Agent、Notebook与取消信号；教育专属session/state只存在于由Teaching Adapter自行Schema验证的Profile Context，不进入通用Kernel字段。Local Adapter已在Web General生产组合根落地，Teaching Adapter仍只有fixture证据。
