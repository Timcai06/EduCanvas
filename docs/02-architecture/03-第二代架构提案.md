...
`TurnApplicationService`核心也已落地为唯一目标编排点：它先固化Context Snapshot，再为每次经Runtime验证的Provider调用结算通用Model Run，所有模型工具调用进入同一Tool Kernel，最后经过Profile确定性复核写入唯一消息终态。它对Context与实际Prompt做防漂移校验，旧Operation replay不再次调用Provider，Trace exporter失败不改变业务结果。Gateway文本Turn已迁入该服务，并由Gateway独占Operation终态；源码暂时仍保留Web General/Web Teaching两处旧Loop。只有两个Web入口也切换并通过扫描后，才满足“生产唯一构造位置”。
...
Gateway runner已经改为投影唯一`TurnApplicationService`；Web General和Web Teaching仍各自装配Context、Prompt、工具、消息账本与回调。入口能力仍会因此漂移：TUI现在已有统一Context/Model Run审计，却还没有Web的Asset与工具集合。后续继续收敛两个Web应用路径，并复用工具/Asset Adapter，不复制Loop。
...
当前已完成生产 `ToolKernel` 与独立 PostgreSQL effect ledger 的第一纵切：四类Adapter使用同一契约，write在执行前创建intention，超时/取消/Adapter显式不确定统一收敛为不可自动重试的`outcome_unknown`。这还不表示入口迁移完成；现有Local/Teaching调用者必须逐条改由Kernel组合，并在真实Gateway/Web故障注入通过后才能删除旧Runtime。

Local AgentTool与Teaching Tool现已有兼容Adapter。通用可信上下文显式传递Operation、Conversation、Trace、Actor、Agent、Notebook与取消信号；教育专属session/state只存在于由Teaching Adapter自行Schema验证的Profile Context，不进入通用Kernel字段。兼容层尚未替换生产组合根，因此不是完成证据。
