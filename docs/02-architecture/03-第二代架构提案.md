...
`TurnApplicationService`核心也已落地为唯一目标编排点：它先固化Context Snapshot，再为每次经Runtime验证的Provider调用结算通用Model Run，所有模型工具调用进入同一Tool Kernel，最后经过Profile确定性复核写入唯一消息终态。它对Context与实际Prompt做防漂移校验，旧Operation replay不再次调用Provider，Trace exporter失败不改变业务结果。当前三条旧入口尚在逐条迁移，因此源码暂时仍保留目标编排点加三处旧Loop；只有Gateway、Web General、Web Teaching全部切换并通过扫描后，才满足“生产唯一构造位置”。
