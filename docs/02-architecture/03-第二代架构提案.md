...
- 负责人：项目负责人
- 最后验证时间：2026-07-21
- 决策状态：研究假设，不是 accepted ADR，不代表已经实现
...
教学审计保留了模型 Run 和 Tool Call 证据，但 `model_runs` 仍被教学 Turn 形状约束。目标不是删除教育证据，而是把通用运行事实提升为通用账本，再由学习事件引用这些证据；教育领域继续拥有判分和掌握度事实，不再拥有模型运行记录的专用副本。
...
- Turn Application Service 的最小稳定输入/输出是否能同时覆盖 Web SSE 和 Gateway NDJSON，而不泄漏传输细节；
- Tool Kernel 的 effect ledger 应与 `gateway_operations`、`tool_calls` 如何建立唯一写者关系；
- 通用模型审计如何从教学形状迁移并保持现有学习证据可追溯；
- 审批 continuation 是优先使用原生 graphile-worker，还是在稳定 Port 后采用 LangGraph；
- Context Snapshot 的保留、删除和未成年人隐私策略；
- Web 与 TUI 的 steer/abort 是否需要完整复刻 OpenClaw 方法，还是保持更小协议。

这些问题仍在研究阶段。未通过决策门前，不创建第二代生产迁移 PR，也不修改 accepted ADR 来伪装结论。
