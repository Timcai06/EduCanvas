...
教学审计保留了模型 Run 和 Tool Call 证据。`model_runs` 已开始以 additive 方式支持通用 `agent_turn`，并保留旧 `teaching_turn` 读取；`tool_calls` 与 Context Snapshot 尚待同样泛化。目标不是删除教育证据，而是把通用运行事实提升为通用账本，再由学习事件引用这些证据；教育领域继续拥有判分和掌握度事实，不再拥有模型运行记录的专用副本。
