...
`educanvas.operation-continuation.v1` 契约与 PostgreSQL 控制账本已经落地。一个 Operation 可以按 `sequence` 依次产生多个等待点，但部分唯一索引保证同一时刻最多一个处于 `waiting_approval / ready / running`；worker 领取使用 owner、到期时间与单调递增 generation 隔离旧进程。账本只保存 Tool Call、approval 和 Adapter 耐久意图的稳定引用，不含 Prompt、正文、工具参数、Credential、Secret、外部回执或副作用结果。批准决策、事件、ready游标与Graphile任务已经原子提交，Worker恢复前会重算当前Agent、Membership、Conversation和approval，最终将continuation与Operation终态原子提交。Turn Application现在会先验证L2/L3参数、落pending Tool Call、要求Adapter幂等准备耐久意图，再发出`approval.required`并无终态挂起；准备失败会诚实失败，绝不执行Adapter副作用。`educanvas.tool-approval-intent.v1`现把Adapter准备与Gateway公开审批隔开：缺少同Operation、同Actor、同过期时间的prepared意图时，审批事件整笔回滚；匹配时事件、Approval、continuation与intent bound在同一事务提交；未绑定且已过期的prepared意图由五分钟有界任务收敛为abandoned。Node与MCP的L0/L1生产Adapter已接线，但尚无L2/L3耐久意图Adapter，因此还不能描述成用户已能批准并续跑高风险动作。
