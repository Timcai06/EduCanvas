...
当前已完成生产 `ToolKernel` 与独立 PostgreSQL effect ledger 的第一纵切：四类Adapter使用同一契约，write在执行前创建intention，超时/取消/Adapter显式不确定统一收敛为不可自动重试的`outcome_unknown`。Web General的Local网页工具已真实经过Kernel并写Tool Call/Effect；Teaching调用者和独立Gateway的共享装配仍待迁移，全部通过故障注入后才能删除旧Runtime。

Local AgentTool与Teaching Tool现已有兼容Adapter。通用可信上下文显式传递Operation、Conversation、Trace、Actor、Agent、Notebook与取消信号；教育专属session/state只存在于由Teaching Adapter自行Schema验证的Profile Context，不进入通用Kernel字段。Local Adapter已在Web General生产组合根落地，Teaching Adapter也已在Web Teaching生产组合根按教学状态白名单接入。
