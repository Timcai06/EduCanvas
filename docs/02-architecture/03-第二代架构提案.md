...
教学审计保留了模型 Run 和 Tool Call 证据。`model_runs` 与 Context Snapshot 已以 additive 方式支持通用 `agent_turn`，并保留旧教学记录读取；`tool_calls` 尚待同样泛化。通用 Context Snapshot 绑定已鉴权Operation，并把实际消息与AssetVersion选择固化为不含正文的可重放证据。目标不是删除教育证据，而是把通用运行事实提升为通用账本，再由学习事件引用这些证据；教育领域继续拥有判分和掌握度事实，不再拥有模型运行记录的专用副本。
