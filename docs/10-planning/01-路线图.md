...
第二代架构以及统一 Canvas 的 U00-U12 已经完成：Source/Artifact 共享 `CanvasResource`、
Renderer Registry、媒体生成闭环和独立 Web Runtime。当前不重写 Agent 内核，重点是修订
U13 Experiment Runtime 契约、验证 V02 实时语音热词、完成对象删除闭环，并开始 General
Agent 产品知识与三层 Memory。
...
- Per-capability Provider 配置（ADR-0021 accepted，配置解析尚未实现）；
- 基于年龄、学科和任务分层的教学质量评测；
