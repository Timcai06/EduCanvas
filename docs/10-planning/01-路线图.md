...
## 当前核心阶段：Canvas Runtime、语音与长期上下文收口

第二代架构以及统一 Canvas 的 U00-U19、语音 V01-V17 已完成工程实现：Source/Artifact
共享 `CanvasResource`、Renderer Registry、媒体生成闭环、独立 Web Runtime、受控
Experiment Runtime、跨入口安全降级，以及受首次模式选择保护的短句输入与课堂字幕。
U20 已按竞赛级范围通过；真人麦克风、课堂噪声、十分钟长流和受控并发作为非阻塞后续
保留，不能对外宣称已验证。生产 Experiment Artifact 数据源仍未接线。当前不重写 Agent
内核，后续继续对象删除闭环、General Agent 产品知识与三层 Memory。
...
### C0：统一资源和 Renderer 边界（已完成）

- Source/Artifact 通用的 `CanvasResource`、Renderer manifest、能力与错误协议已经落地；
- Renderer Registry 已拆开协议、资源解析、具体 Renderer 与页面组合；
- Studio 以输入/输出组织同一 Notebook 的 Sources 与 Artifacts；
- 保持现有 Artifact、HTML 沙箱和学习事件兼容。

### C1：来源阅读与多模态输入（基础已完成，继续扩展）
...
### C2：多模态生成与共创（媒体闭环已完成，Provider 配置待统一）
...
### C3：隔离交互应用（Tier 2 已完成）

- 轻量 HTML 预览与持久、版本化的探索型 Artifact 保持独立生命周期；
- 独立 Runtime、版本化消息桥、资源策略、取消、错误和恢复协议已经落地；
...
- Personal、Notebook、Conversation 三层 Memory，带来源、版本、删除、共享和不可用状态（ADR-0020 accepted，M0/M1 尚未实现）；
- General Agent 产品知识与通用 Notebook RAG，复用现有检索基础设施但使用独立作用域、版本和授权；
- Per-capability Provider 配置（ADR-0021 accepted，配置解析、继承与能力级禁用已实现）；
- 基于年龄、学科和任务分层的教学质量评测；
