...
## 已完成核心阶段：Canvas Runtime 与语音收口

第二代架构以及统一 Canvas 的 U00-U19、语音 V01-V17 已完成竞赛级收口：Source/Artifact
共享 `CanvasResource`、Renderer Registry、媒体生成闭环、独立 Web Runtime、受控
Experiment Runtime、跨入口安全降级，以及受首次模式选择保护的短句输入与课堂字幕。
U20 的自动化边界已通过，但真人麦克风、课堂噪声、十分钟长流和受控并发仍保留为非阻塞
后续；生产 Experiment Artifact 数据源仍未接线。当前不重写 Agent 内核，后续继续对象
删除闭环、General Agent 产品知识与三层 Memory。

稳定边界见[统一 Canvas 工作面](../02-architecture/04-统一画布工作面.md)、
[ADR-0009](../09-decisions/0009-统一画布工作面与运行时分层.md)与
[UV 归档](../plan/completed/UV-画布语音.md)；已完成的
[F 画布界面](../plan/completed/F-画布界面.md)作为既有外壳与可访问性基线。
