...
- 收敛 `make all`、`make dev`、`make tui`，完成 Web-first 与交互式 TUI 的低摩擦本地入口；
- 统一 Web/TUI registered identity、默认 Personal Workspace 和一次性双向 handoff；
- 建立 Web Connections 与 TUI `/channels` 的 provider-neutral 自助连接控制面；
