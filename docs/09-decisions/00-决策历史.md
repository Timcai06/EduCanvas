...
## 2026-08-10 至 2026-08-13：桌宠从语音附件演进为统一桌面外延

- ADR-0024 先确定 Electron + React、独立桌面 2D 角色、显式点击语音、系统浏览器登录、
  唯一 Agent Runtime、最小桌面权限和 Web/Canvas handoff，为 P1-P3 MVP 建立边界；
- MVP 随后落地透明窗口、拖动与托盘、文本和整段语音问答、TTS、可撤销桌面身份、
  `gateway.v1` 接入、折叠对话框和独立聊天窗口；
- 实践表明“固定单一 Conversation、只显示短气泡、不显示完整历史、复杂结果自动打开 Web”
  过窄，也与用户已经确认的桌面聊天需求冲突；macOS 优先顺序也与当前 Windows 开发和
  打包证据不一致；
- ADR-0028 吸收 ADR-0024 仍有效的安全、Runtime、Electron、角色与语音边界，并把桌宠重新
  定义为统一 EduCanvas 系统的能力受限第一方桌面外延；
- 新方向要求服务端 Conversation/Message 权威、身份与会话游标解耦、真实 Operation 事件、
  结构化结果和用户点击式精确 handoff，同时保留现有 MVP 渐进迁移，不推倒重写；
- ADR-0024 因被完整替代而删除，具体实现差距、测试基线和迁移顺序见 DP00 质量记录与 DP 计划。
