...
- 2026-08-07（W 线收口）更新：Web Renderer Registry（`canvas-resource-registry`）已实现，
  按 `rendererId@rendererVersion` 选择预注册 Renderer，不匹配时返回稳定 unavailable 语义；
  Artifact 内容分发已迁入 Registry——mind_map/slides/flashcards/note/dom 5 类内容驱动
  Renderer 渲染真实内容，交互式 kind 构造抛错并选择 unavailable。仍待：Source 打开尚未
  迁移 Registry；Artifact 的 `rendererId` 服务端字段缺失，浏览器端以 kind→rendererId
  映射兜底（Issue #306，R 线接口补全后 `artifact-canvas-resource.ts` 可删除）。
