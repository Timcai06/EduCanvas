...
ADR-0001确定核心后端与Next.js分离（主选NestJS + Fastify），同时指出"可以从同一Monorepo逐步拆分"。竞赛阶段一的首要风险是闭环无法按时跑通，而不是扩容能力不足。`docs/05-engineering/backend.md`还遗留了数据访问层在Kysely与Drizzle之间的待定项。
...
- 阶段一的API暂由Next.js Route Handler承载，`/api/v1`约定与错误结构仍按`docs/05-engineering/api-conventions.md`执行；
- 阶段二拆分core-api时需要一次接口迁移，届时补充新的ADR记录拆分边界；
- `docs/05-engineering/backend.md`的数据访问待定项由本ADR关闭。
