...
**第 2 步：把仓库克隆到你电脑上。** 打开终端（Windows 可以使用 WSL2，也可以使用下文的原生一键入口），执行：
...
make dev                 # 启动数据库、补齐迁移并加载.env启动项目
```
...
不使用 WSL2 的 Windows 队友先安装 Node.js 22、Git 和 Docker Desktop，在
PowerShell 中执行 `corepack enable`、`pnpm install --frozen-lockfile`，并把
`.env.example` 复制为 `.env`。之后双击仓库根目录的 `Start EduCanvas.cmd`
即可启动 Web 验证环境。窗口必须保持打开；按 `Ctrl+C` 会让统一 orchestrator
停止它本轮拥有的 Web、Gateway 和 Worker。这个入口与 `make dev` 共用同一套
服务完整性检查，不应在 PowerShell 脚本里另写 Web-only 健康检查或直接启动
`pnpm dev:core`。
