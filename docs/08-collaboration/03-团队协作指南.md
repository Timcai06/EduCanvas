...
**第 3 步：装开发环境。** 你只需要装三样东西：**Node.js 24.18.0（Node 24）、Docker Desktop、Git**。装好后在仓库目录里依次执行：

```bash
corepack enable          # 让电脑自动使用项目指定的pnpm版本，不用自己装
pnpm env:init            # 创建本地配置；Provider 可选，已有 .env 时拒绝覆盖
pnpm env:check           # 校验本地配置，不打印密钥
...
不使用 WSL2 的 Windows 队友先安装 Node.js 24.18.0（Node 24）、Git 和 Docker Desktop，在
PowerShell 中执行 `corepack enable`、`pnpm install --frozen-lockfile`，再运行
`pnpm env:init` 创建 `.env`。之后双击仓库根目录的 `Start EduCanvas.cmd`
即可启动完整 core（Web/Gateway/Worker + PostgreSQL 迁移）。窗口必须保持打开；
按 `Ctrl+C` 会让统一 orchestrator 停止它本轮拥有的 Web、Gateway 和 Worker。
这个入口与 macOS/Linux 的 `make all` 共用同一套统一启动状态机
（`tooling/local/local-orchestrator.mjs`），不应在 PowerShell 脚本里另写
Web-only 健康检查或直接启动 `pnpm dev:core`。
...
- API Key、密码、Token、私钥；
- `.env` 文件（真实配置只放自己电脑上，仓库模板位于 `config/env/local.env.example`）；
- 学生真实个人信息；
- 大文件：模型权重、构建产物、临时缓存；
- 没确认过版权授权的教材、图片、视频。
...
本指南以本文件（`docs/08-collaboration/03-团队协作指南.md`）为唯一事实源，不在仓库里保存 PDF 副本——需要 PDF 时从本文件现场导出即可。
