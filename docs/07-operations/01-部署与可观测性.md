...
指标管线是进程内低基数注册表（`packages/telemetry/src/metrics.ts`），不依赖 Collector/Exporter：`GET /v1/internal/metrics` 在 `observability` 计数之上返回 `telemetry.health` 与 `telemetry.metrics` 快照；Collector 未配置时 health 明确为 `disabled`（配置/初始化失败为 `degraded` + 稳定 failureCode）。指标名与标签键是声明闭集（`TURN_METRIC_DEFINITIONS`，15 个），标签值强制低基数短串，任何用户 ID、正文、URL、对象路径、Secret 或原始错误都被拒绝；业务事实仍以 DB 账本为准，Metrics 不替代 Turn/Operation/Learning 记录。
