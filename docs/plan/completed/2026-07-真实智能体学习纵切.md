...
| 事项                                                           | 去向                                                                                                             |
| -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| PR-C1（Artifact 提议/确认/生成/真实 Studio）                   | 升格为通用 Artifact Runtime，由[后续计划](2026-07-GeminiNotebookLM复刻.md) M1 吸收，不再按 K12 专用路径开发 |
...
当时的 Provider/账本决策已验收；当前仍有效的约束见[决策历史](../../09-decisions/00-决策历史.md)与现行 ADR。协议、数据、架构事实已回写 `docs/02-architecture/`、`docs/04-data/`、`docs/05-engineering/` 与 README/技术报告。
