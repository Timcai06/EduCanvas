...
- 代码审核与最终验收：Codex
- 最后验证时间：2026-08-08
- 归档说明：Q00-Q07 全部 PASS；Q01 修正版 #307 已合并（2026-08-07）；Q02 最终验收在 W/Q/R 收口 PR 完成（provider degradation reason 端到端接线）；Q05/Q06 门禁随 #303/#304 合并生效；Q07 报告 #305 已合并（2026-08-08）
- 并行计划：[R 运行时收敛](../completed/R-运行时事实收敛.md)、[W 工作面画布](W-工作面画布收敛.md)
- 后续出口：[G 产品发布闭环](../active/G-产品发布闭环.md)
- 关联计划：[KM 知识记忆](../active/KM-知识记忆.md)
...
| 任务            | 状态   | 证据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q00 指标冻结    | `PASS` | 指标组与阈值语义冻结于本计划 Q00 节（Retrieval/Answer/Runtime/Latency/Cost/UI-Test/Privacy 与 CI gate/趋势/人工评审分类）；文档任务无独立 PR；汇总见 docs/06-quality/09-质量基线报告.md（2026-08-07 回写）                                                                                                                                                                                                                                                                                                                 |
| Q01 RAG eval    | `PASS` | #288 合入后被 #293 revert（延迟基线混入多检索器配置）；修正版 #307 已合入（2026-08-07），评测走 @educanvas/db 受控入口 + 报告移出 git；**最终复现通过**（2026-08-07，docker pgvector 隔离库 educanvas_eval_test，2/2 用例 1.07s，含 fallback 诚实标记验证）；复现链路修复：root vitest 链接 + alias 正则形式 + drizzle 前缀匹配（PR 待合）                                                                                                                                                                                 |
| Q02 降级观测    | `PASS` | #289 已合入（a7df105）；9 reason 冻结于 agent-core + 三侧分类 + teaching-runtime `retrieval_degradations` 指标；**断线已修复**（0ea4d80，PR #313）：指标注册进 Q04 注册表（reason 9 值闭集）+ `knowledge-retrieval-runtime` 双通道记录 + teaching-runtime 补导出 `recordRetrievalDegradation`；**最终验收通过**（2026-08-07：db 集成测试 47 文件 295 用例全绿，覆盖 5 个 reason 场景；web/telemetry tsc 通过）                                                                                                             |
| Q03 Turn budget | `PASS` | budget controller + ledger（#291，已合入 main 01424d4）；注：合入于 2026-08-06 Actions 故障期，head 最终 check 含 e2e failure，main 现已回归全绿（2026-08-07）                                                                                                                                                                                                                                                                                                                                                             |
| Q04 SLO/Runbook | `PASS` | metrics registry + Web/Gateway/Worker 唯一组合层包装 + internal metrics 端点 + SLO/Runbook（#295，已合入 main 0085e06；合入期 head 最终 check 含 failure/skipped，Actions 故障期影响，main 现已回归全绿）；指标故障旁路业务结果，协议型标签值使用闭集                                                                                                                                                                                                                                                                      |
| Q05 测试真实性  | `PASS` | coverage 门禁（telemetry 92/90/83/92、agent-core 96/94/96/97、agent-runtime 78/74/86/80，2026-08-06 基线）+ chromium-mobile 第二环境进默认 lane + hydration 检查 + bundle/route size 基线（jsTotal 4120016B/entry 875476B）+ @ui 独立 lane（chromium+firefox，nightly+路径触发）+ retry/flaky 汇总进 CI Summary；#303 已合入 main `88027be`（分支 CI 3 次全绿）；main 回归待恢复（size gate 基线过时：#304 已重录基线随 `d01db29` 合入，#309 另录精确基线待合并，见报告 §7-3）                                             |
| Q06 供应链发布  | `PASS` | #304：action 40 位 SHA pin + dependency-review 门禁（high 即 fail）+ 容器 digest pin + migration records 门禁（51 迁移全字段）+ release evidence 语义（passed/skipped/failed 终态语义）+ 供应链文档 08-供应链与发布证据.md；#304 已合入 main `d01db29`，分支最终 run（#31161098720）**8 项终态全绿**（dependency-review/secret-scan/checks/integration/windows/runtime-pressure/e2e SUCCESS + release-evidence SKIPPED 预期，rc1 未发布）；main 回归待恢复（dependency-review push-event 缺陷，修复 PR #311，见报告 §7-5） |
| Q07 收口        | `PASS` | docs/06-quality/09-质量基线报告.md（2026-08-07-2，按 Code Owner 审核意见修订：job 清单/数量/基线/状态语义/门禁接线修正）+ 本文台账回写；PR #305 已合并（2026-08-08）；Codex 独立复核通过后完成归档                                                                                                                                                                                                                                                                                                                         |
