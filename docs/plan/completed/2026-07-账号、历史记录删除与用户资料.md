...
- 负责人：项目负责人
- 完成时间：2026-07-24
- 实现：PR #192 / commit `293b54a`
- 架构决策：[ADR-0008](../../09-decisions/0008-Web账号身份与会话边界.md)

## 归档结论

本阶段已交付用户名注册/登录/退出、受控 session、昵称和头像资料、修改密码与
本人聊天历史归档；匿名用户继续可用，匿名历史不自动迁移。原始规格保留在后文，
作为这次交付的验收清单。
...
| 验收项 | 可复现证据 | 结果 |
| --- | --- | --- |
| API、密码、session 与头像边界 | `apps/web/app/api/v1/auth/**/*.test.ts`、`apps/web/app/api/v1/me/**/*.test.ts`、`apps/web/server/auth/*.test.ts`、`apps/web/server/profile/avatar.test.ts` | `passed in PR #192` |
| 本人历史归档 | `apps/web/app/api/v1/chat/conversations/[conversationId]/route.test.ts` | `passed in PR #192` |
| 数据持久化 | `packages/db/src/web-account-repository.integration.test.ts`、迁移 `0035_curvy_justin_hammer.sql` | `passed in PR #192` |
| 浏览器闭环 | `tests/e2e/account-flow.spec.ts` | `passed in PR #192` |

长期身份与会话约束由 ADR-0008 持有；邮箱/OAuth、自动迁移匿名历史、物理删除
关联业务数据和公开头像 URL 均不属于本阶段，后续必须单独立项。
