...
- EduCanvas SSE v1 自有流式协议；真实 DeepSeek Provider 原生 SSE Adapter，无 Provider 时诚实失败（ADR-0007）；
- Turn/Message/Model Run/Tool Call 账本、promptHash 审计、取消/租约/幂等与刷新恢复（ADR-0008）；
- 两阶段工具循环（answer → tools → synthesis），后续由 ADR-0011 修正为容忍工具前导文本；
...
| 事项 | 去向 |
| --- | --- |
| PR-C1（Artifact 提议/确认/生成/真实 Studio） | 升格为通用 Artifact Runtime，由 [ADR-0012](../../09-decisions/0012-artifact-runtime-durable-jobs.md) 与[新计划](../active/2026-07-gemini-notebooklm-replica.md) M1 吸收，不再按 K12 专用路径开发 |
...
ADR-0007/0008 accepted；协议、数据、架构事实已回写 `docs/02-architecture/`、`docs/04-data/`、`docs/05-engineering/` 与 README/技术报告。
