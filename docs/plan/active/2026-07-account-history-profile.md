...
- 注册账号并登录。
- 使用 8–128 位密码，并在输入时看到 3 档密码风险等级。
- 删除自己的历史记录。
- 上传自己的头像。
- 修改自己的昵称。
- 修改自己的密码。
...
密码最少 8 位、最多 128 位。超出范围时不能提交。

UI 显示 3 档风险等级：

- 高风险：长度至少 8，但只有一种字符类型。
...
服务端必须重新校验密码 8–128 位；前端强度提示只用于体验，不能作为安全边界。
...
1. local 部署始终返回与 TUI/Gateway 共用的配置主体，默认 `local:owner`。
2. 非 local 部署存在有效登录 session 时返回注册用户身份。
3. 否则回退到当前匿名身份逻辑。
...
1. 首版账号标识使用用户名，不引入邮箱服务。
2. 注册后不自动迁移匿名或 local 历史。
3. 历史记录使用软删除/归档。
4. 头像通过受控 route 读取，不提供公开对象 URL。
5. 账号主体、密码事务、限流与 local 兼容边界以 ADR-0023 为准。
