...
| ADR-0018 仍为 proposed，且原生 addon、热词、真麦克风未验证 | `docs/09-decisions/0018-实时语音输入选型与流式识别边界.md`                                                                           | V01-V03 是语音产品接线硬门槛              |
...
| ------------------------------ | ----------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------- |
| S0 基线与语音可行性            | U00 → V01 → V02 → V03                                       | U01 可在 V01-V03 期间独立进行                                             | V03 接受 ADR-0018，U01 通过   |
...
```text
只根据 V01/V02 的真实证据更新 ADR-0018。记录原生 addon、热词、尾部静音、仍未验证项
和选型后果。由项目负责人决定 accepted 或保持 proposed/blocked；DeepSeek 不能自行接受。
未 accepted 时明确阻塞 V04 及以后所有语音实现。
```

完成标准：

- ADR 状态与证据一致，保留真麦克风、长流和并发未验证项；
- accepted 必须有项目负责人确认；否则依赖图明确停止；
- 不修改代码。
...
| 能力                       | 当前状态  | 当前证据                                      | 完成任务                  |
| -------------------------- | --------- | --------------------------------------------- | ------------------------- |
| CanvasResource/manifest    | `passed`  | canvas-protocol 源码与测试                    | 已有，U01 防回归          |
| Source/Artifact 服务端投影 | `passed`  | server adapter 与统一 route                   | 已有，U01 防回归          |
| Web 统一 registry/打开     | `pending` | 主 Canvas 仍消费 PublicArtifact               | U02-U05                   |
| 媒体生成闭环               | `partial` | 图像 Provider/checkpoint 已有，完整证据待审计 | U06-U08                   |
| 持久 Web Runtime           | `pending` | 只有轻量消息预览                              | U09-U12                   |
| Experiment Runtime         | `pending` | 未发现 ExperimentRuntimePort                  | U13-U16                   |
| TUI/渠道 Canvas            | `pending` | 未发现统一 CanvasResource 消费                | U17-U19                   |
| 原生流式识别与热词         | `blocked` | ADR-0018 记录未跑通                           | V01-V03                   |
| 流式 Port/Gateway/UI       | `pending` | 仅有一次性转录 Port 和禁用按钮                | V04-V09、V12-V13、V16-V17 |
| 音频同意与可靠删除         | `pending` | 有通用 guardian/outbox，缺音频专用契约        | V10-V15                   |
