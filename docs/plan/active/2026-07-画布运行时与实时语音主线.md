...
**S2 状态：** U06-U07 passed；媒体 Provider/版本/投影、文本等价、受控下载与可靠删除均已复核。
由 U08 补齐完整 Operation 终态与恢复证据后才能收口。
...
| 媒体生成闭环               | `partial` | U06 已确认现有图像/音频 Provider、版本、元数据、净化 provenance、受控读取与 checkpoint 基础闭环；U07 已通过文本等价/下载删除访问复审；U08 的完整终态恢复证据仍待完成 | U06-U07 passed；U08 待完成 |
