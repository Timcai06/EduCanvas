...
| ADR-0018 已 accepted，但原生 addon、热词、真麦克风仍未验证 | `docs/09-decisions/0018-实时语音输入选型与流式识别边界.md`                                                                           | 选型已确定；V01-V03 仍是产品接线硬门槛    |
...
```text
复现 ADR-0018 的原生 addon 空文本问题。使用 16 kHz 单声道、无个人信息的合成 WAV，
建立最小脚本，找出 sherpa-onnx-node 与 WASM API 差异，让原生 addon 返回非空中文。
记录 Node/arm64/package/model 版本、命令、输出和 RTF。模型权重及 WAV 不提交。
无法解决时保留最小复现与错误证据并标记 blocked，不得回退为未说明的 WASM 产品方案。
```

完成标准：

- Apple Silicon 原生 addon 输出正确的非空中文；
- 同一 fixture 有原生/WASM RTF 对照，脚本不含本地绝对模型路径；
- 未提交模型、音频、二进制或依赖缓存；失败时没有继续 V02。
...
```text
只验证热词，不接业务。基于 V01 原生脚本配置 hotwordsFile、hotwordsScore、
...
选型，记录原生 addon、热词、尾部静音和仍未验证项；accepted 不等于产品能力已启用。
若原生 addon 或热词仍未跑通，在本计划把能力门禁标为 blocked 并阻塞 V04 以后任务。
```
...
```text
用 V01/V02 验证过的原生 API 实现 StreamingTranscriptionGateway adapter。
...
- 原生 addon/热词、真麦克风、长流、并发、撤回/到期硬删除均有真实证据；
...
**基线 Commit：** `dfec9849e1efee638f636e33041b054e2bdfd7aa` (2026-07-28)

**S0 状态：** Canvas 子线 passed（U00/U01），语音子线 blocked（V01-V03）；S0 整体未通过，
Canvas 后续任务可按依赖图独立推进。

**S1 状态：** Canvas 子线 passed（U02-U05）；语音任务仍受 V01-V03 阻塞，不得提前进入 V04。

| S0 任务 | Codex 结论 | 证据                                                                                              |
| ------- | ---------- | ------------------------------------------------------------------------------------------------- |
| U00     | `PASS`     | 基线事实、提交与验证映射已复核                                                                    |
| U01     | `PASS`     | Canvas protocol 58/58、Web 385/385、两侧 typecheck 通过；资源访问边界测试覆盖身份与 Notebook 参数 |
| V01     | `BLOCK`    | 官方 Node addon 文件识别配置、全精度模型、0.4 秒尾部静音下，四个官方 WAV 均返回空文本             |
| V02     | `BLOCK`    | 依赖 V01                                                                                          |
...
| U03     | `PASS`     | Source/Artifact 的 Studio、对话产物卡与生成状态卡统一先读取 CanvasResource 并进入 registry；Notebook 切换取消旧请求，Artifact 保留受控详情兼容链  |
| U04     | `PASS`     | SourceResourceRenderer 已接入生产打开链；覆盖 loading/empty/failed/unavailable/denied/ready、真实重试、aria-live、替代文本及 PDF/PNG fixture 定位 |
| U05     | `PASS`     | 隔离 E2E 2/2：统一打开、跨 Notebook/用户 404、v2→v1→v2、旧 Source/Artifact URL 和 PublicArtifact 判分均通过                                       |
...
| 媒体生成闭环               | `partial` | `apps/worker/src/tasks/image-artifact-generation.ts:17-188` 有 Provider/checkpoint，完整证据待审计                                     | U06-U08                   |
| 持久 Web Runtime           | `pending` | `apps/web/features/canvas/sandbox-preview.ts:1-75` 只有轻量消息预览                                                                    | U09-U12                   |
| Experiment Runtime         | `pending` | 未发现 ExperimentRuntimePort                                                                                                           | U13-U16                   |
| TUI/渠道 Canvas            | `pending` | 未发现统一 CanvasResource 消费                                                                                                         | U17-U19                   |
| 原生流式识别与热词         | `blocked` | ADR-0018 记录未跑通；`tooling/voice-lab/` 提供手工复现工具，模型与 WAV 仅保存在本地且被忽略，V01 确认返回空文本                        | V01-V03                   |
...
| 能力         | ADR 状态 | 代码状态    | 阻塞原因                                      |
| ------------ | -------- | ----------- | --------------------------------------------- |
| 原生 addon   | accepted | **blocked** | 原生 addon 返回空文本（V01 已验证，根因未知） |
| 热词         | accepted | blocked     | 依赖原生 addon                                |
...
- 环境: sherpa-onnx-node 1.13.4, Node.js v24.18.0, arm64
- 模型: sherpa-onnx-streaming-zipformer-bilingual-zh-en-2023-02-20
- 方法: 官方 Node addon 文件识别配置、全精度模型组合与 0.4 秒尾部静音
- 结果: OnlineRecognizer 创建成功，0.wav、1.wav、2.wav、3.wav 均返回空文本，根因未知
- 阻塞: V02-V03, V04-V09, V12-V13, V16-V17 全部 blocked
