...
资源策略均已通过 Codex 安全复审。U12 已解锁但尚未实现；浏览器隔离、服务端重授权与
非合作负载门禁仍必须由 U12 证明。
...
| 持久 Web Runtime           | `contracted` | ADR-0019、U10 消息/Port 契约与 U11 依赖/资源策略已通过；`sandbox-preview.ts` 仍只是轻量 srcdoc 预览，真实隔离 Adapter 尚未实现         | U09-U11 passed；U12 pending |
