...
**基线 Commit：** `d45e8a37221032b7e33145bc5d30d5a41ecbeec7` (2026-07-28)

**S0 状态：** Canvas 子线 passed（U00/U01），语音子线 blocked（V01-V03）；S0 整体未通过，
Canvas 后续任务可按依赖图独立推进。

**S1 状态：** U02 passed，U03 已解锁；语音任务仍受 V01-V03 阻塞，不得提前进入 V04。
...
| U03     | `READY`    | 依赖 U02 已满足；下一步只迁移统一打开链路和兼容适配，不修改服务端授权与旧 PublicArtifact 判分路径       |
...
| Web 统一 registry/打开     | `partial` | U02 registry 已通过；`canvas-panel.tsx` 仍消费 PublicArtifact，统一打开链路与兼容适配待 U03-U05                                        | U02 passed；U03-U05 待办  |
