...
- 代码审核与阶段验收：Codex
- 最后验证时间：2026-08-10
- 下一领取任务：`U21` 全量门禁与归档；`U20` 已按竞赛级范围通过
...
| Composer 语音入口受首次模式选择与 Gateway 能力闸门控制                          | `apps/web/features/composer/composer.tsx`、`apps/web/features/voice/`                                                                | V17 已完成短句输入与课堂字幕接线             |
...
| S3 产品接线与持久 Runtime      | V12 → V13 → V16 → V17；V14 → V15 → V17；U09 → U10/U11 → U12 | V12 可与 V14、U09 并行；U10 与 U11 并行                                   | V17、U12 通过，入口受模式与能力闸门控制 |
...
- 状态：`PASS`（Codex 于 2026-08-10 完成实现与竞态复核）
- 依赖：V07、V13
...
- 状态：`PASS`（2026-08-10；产品 UI、BFF、运行时能力闸门与 fixture E2E 已复核）
- 依赖：V13、V15、V16、V05
...
首次打开先选择模式：限制模式允许瞬时云端识别但不留存原始音频；通用模式由使用者
一次确认已有监护人同意。该模式选择是竞赛演示的产品偏好，不是监护关系证明。
实时入口仅在模型 adapter 与 Gateway 连接配置均健康时开放；任何原始音频留存仍走
V11/V14/V15 的独立同意、留存与删除契约。
权限拒绝、无设备、连接失败、模式未选择和能力关闭必须可读；浏览器不存音频。
```
...
状态：`PASS`（2026-08-10）。Gateway Core/Client、Web、TUI 与 Telegram
复用 Operation fixture，覆盖取消、能力不可用、Runtime/内部失败、恢复游标、未知事件拒绝
和终态后拒绝；Telegram 对控制事实作有界安全降级。Canvas 的跨 Notebook/主体拒绝沿用
公共 `CanvasResource` 投影测试；Voice 的 idle/duration 超时沿用流式通道配额测试。修复 Web
语音票据 BFF 将 Gateway 统一 404 错误抹成 503 的偏差，且不透传下游响应体。
...
状态：`PASS`（2026-08-10，竞赛级范围）。自动化边界与本机限制记录见
[UV 真实环境验收记录](../../06-quality/10-UV真实环境验收.md)。项目负责人明确将真人
麦克风、课堂噪声、十分钟连续流、受控实时并发和可选留存删除实跑降为非阻塞后续。
这些项目仍无本轮真实证据，不得对外宣称已经验证，也不再阻挡 U21 与竞赛阶段归档。
...
- [ ] 首次使用必须选择模式；缺模型或 Gateway 连接时实时语音不能进入产品；
- [ ] 任何原始音频留存仍要求 V11/V14/V15 的同意、留存与删除闭环；
...
恢复闭环；V17 已完成短句/课堂字幕产品接线、首次双模式选择与服务端基础设施闸门。
限制模式允许只在内存和实时链路中处理 PCM 的云端识别，不创建音频 Asset/Retention；
通用模式的一次确认只用于竞赛演示产品分流，不冒充可审计监护证明。实时入口只有 Gateway
实际报告流式适配器可用且连接配置成立时才开放；若未来保留原始音频，仍由 V11/V14/V15
的同意、留存和删除契约独立 fail closed。
...
不执行 Runtime，也不发送正文、Prompt、对象键或长期链接。V17 已完成语音产品接线，S4 阶段
出口已满足；真实模型、真麦克风和长流质量仍属于联合验收证据，不由 fixture 冒充。

> 2026-08-10 状态校正：下表保留各原子任务复审时的详细过程证据；其中 V12、V13 行内的
> `REVIEW_REQUIRED`、"未提交/等待复审"与 V17-A 的"完整 V17 尚未实施"均是当时阶段状态。
> 这些项现已随 V17 合并事实收敛为 `PASS`；V02-X 仍是非阻塞可选增强。当前唯一发布阻塞
> 是 U20 真人环境验收，不再使用“产品语音门禁默认关闭”的旧描述。
...
| V16     | `PASS`     | 浏览器采集保持 SSR 安全且只在显式启动后请求麦克风；16/44.1/48 kHz 到 16 kHz mono、PCM16LE、chunk 上界与连续 sequence、权限/无设备/AudioContext/consumer 失败、stop/cancel/cleanup 竞态及无网络无持久化均有测试。Codex 复核 4 个 capture/PCM 文件与相邻组合测试，语音目标集 10 files/166 tests、Web typecheck 与 Prettier 通过。                                                                                                                                                                                                                                                                                                                                                                                            |
| V17-A   | `PASS`     | 浏览器 transport 复用 V05 reducer 与 V07 wire 契约；一次性 ticket 只进入 WebSocket 子协议，长时 bearer 只允许同源 ticket 请求，未知服务端事件 fail closed。Codex 返工修复 Web Crypto 方法 receiver、观察者异常破坏状态机、发送/socket 错误后连接引用未释放，以及控制器 disconnect 异常阻断清理；49 条 transport 与 27 条 controller 回归通过。完整 V17 的 UI/BFF/E2E 尚未实施。                                                                                                                                                                                                                                                                                                                                            |
| V17     | `PASS`     | General/Learning Composer 接入短句与课堂字幕：短句仅在唯一 final 后调用既有 `onSend` 一次，字幕只追加 segment final、零 Turn。首次打开在限制/通用模式中二选一；限制模式允许瞬时云端识别且不留存音频，通用模式要求一次监护人同意确认，但 Cookie 仅是竞赛演示偏好、不是监护关系证明。BFF 每次签票前重查 Gateway 运行时适配器与连接配置，再由服务端 bootstrap 换取绑定主体+Notebook 的单次 ticket；长时 bearer 不出服务端，浏览器不持久化 PCM。若产生原始音频留存，仍必须独立通过 V11/V14/V15。模式/能力/签票目标测试 8 files/33 tests、Web/E2E typecheck、Web lint、webpack production build 与 file governance 通过；fixture E2E 因本地未配置独立 `E2E_DATABASE_URL` 留给远端 CI，真实模型/真麦克风/长流未由 fixture 冒充。 |
...
| TUI/渠道 Canvas            | `passed`  | Gateway 提供按 bearer 主体和 Notebook 重新授权的 CanvasResource 目录；TUI `/canvas` 使用一次性交接，Telegram 按可信绑定输出有界摘要且不执行 Runtime                                                  | U17-U18 passed；U19 已解锁 |
| WASM SIMD 流式识别与热词   | `passed`  | V01 已选定 WASM SIMD；本地与云端的专业术语质量缺口由负责人明确接受为非阻塞风险，原始证据仍保留                                                                                                       | V04-V09                    |
| 流式 Port/Gateway/UI       | `passed`  | V04-V09 的领域契约、reducer、分段策略、Adapter 与模型配置已接入 Gateway 受鉴权 WebSocket、短句/课堂字幕 UI、首次双模式选择和两项基础设施闸门；真实模型与真麦克风仍留给联合验收，不混同于工程接线通过 | V12-V13、V16-V17           |
| 音频同意与可靠删除         | `passed`  | V10/V11/V14/V15 已完成同意、留存 Repository、审计、删除意图与可靠硬删除闭环                                                                                                                          | —                          |
| 联合发布证据               | `pending` | U20 已按竞赛级范围通过；等待 U21 唯一一次全量 release-evidence 门禁与归档                                                                                                                            | U21                        |
...
| 11  | Composer 语音入口受模式选择与基础设施闸门控制   | `apps/web/features/composer/composer.tsx`              | —       | 未选择模式或缺模型/连接时关闭；健康时开放短句/课堂字幕                |
...
- 决策: V02-S/V02-T/V02-U/V02-W 的失败证据全部保留；项目负责人于 2026-08-05 接受专业术语质量风险，采用“WASM 本地草稿 + 可选云端复核”双路径。V02/V03 passed，V04-V17 已按依赖完成工程接线；项目负责人于 2026-08-10 将真人长流矩阵降为非阻塞后续，U20 按竞赛级范围 PASS
