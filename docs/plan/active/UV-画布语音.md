...
恢复闭环；V17 已完成短句/课堂字幕产品接线与服务端总闸门。入口仍默认关闭，只有 Gateway
实际报告流式适配器可用、有效同意、留存 Repository、删除 Worker 与连接配置全部成立时才开放。
...
| V17     | `PASS`     | General/Learning Composer 接入短句与课堂字幕：短句仅在唯一 final 后调用既有 `onSend` 一次，字幕只追加 segment final、零 Turn。BFF 每次签票前以可信 Web 主体重查 Gateway 运行时适配器状态、同意/Repository 与删除 Worker 闸门，再由服务端 bootstrap 换取绑定主体+Notebook 的单次 ticket；长时 bearer 不出服务端，浏览器不持久化 PCM。权限、设备、连接、能力关闭和撤回均有可读状态。复核证据：Web 全套 136 files/1073 tests、语音目标 7/50、DB 同意闸门 1/27、Gateway HTTP 1/6、fixture E2E 1/1、Web/Gateway/DB/E2E typecheck 与 webpack production build 通过。真实模型/真麦克风/长流未由 fixture 冒充，入口默认关闭。 |
...
| 流式 Port/Gateway/UI       | `passed`  | V04-V09 的领域契约、reducer、分段策略、Adapter 与模型配置已接入 Gateway 受鉴权 WebSocket、短句/课堂字幕 UI 和五维服务端总闸门；真实模型与真麦克风仍留给联合验收，不混同于工程接线通过 | V12-V13、V16-V17           |
...
| 11  | Composer 语音入口受五维能力总闸门控制           | `apps/web/features/composer/composer.tsx`              | —       | 缺任一能力时禁用并显示稳定原因；全健康时开放短句/课堂字幕             |
