...
| L05 音频与字幕时钟      | `REVIEW`  | 已增加逐帧消费 ACK 与有界 PCM 窗口；Provider finished 等待最后 ACK 后才关闭。空/奇数/乱序/重复 PCM fail closed，播放取消不会产生幽灵 source；字幕和播放游标只由 Web Audio 排期推进，并以跨轮真实 PCM 时长校准估算。待远端 CI 与独立 reviewer 判定 PASS。 |
