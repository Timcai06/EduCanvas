...
| 任务          | 状态      | 交付与验收                                                                                                                                                                                                                           |
| ------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| LC00 事实冻结 | `PENDING` | 代码审计完成，记录 delta、TTS、Artifact、Runtime 当前链路；建立 fixture 延迟基线和失败矩阵；禁止把已实现能力重复建设。详见 [LC00-LC01-事实冻结与契约矩阵.md](../../06-quality/12-LC基线与契约矩阵.md)。                              |
| LC01 契约矩阵 | `PENDING` | 固定 Live 游标、输出意图、Artifact kind/version、Renderer/Runtime 和错误码所有权；列出旧 `canvas`、`note`、`dom_exploration` 的兼容期限和迁移路径。详见 [LC00-LC01-事实冻结与契约矩阵.md](../../06-quality/12-LC基线与契约矩阵.md)。 |

### 6.2 Live 实时交互线

| 任务                    | 状态      | 交付与验收                                                                                                                                                                 |
| ----------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L01 同源流式消息        | `PENDING` | Live 面板直接消费当前 Assistant `message.delta` 投影，不维护第二份完整回答；普通聊天同一消息逐增量增长，入室/出室均不丢字、不重复字。                                      |
| L02 三游标状态机        | `PENDING` | 引入 `displayCursor`、`speechCommittedCursor`、`audioPlayedCursor` 和单调 `runId`；覆盖消息 ID 替换、增量回退、重复 delta、终态后事件和重新入室。                          |
...
| 验收项               | 证据                                         | 结果      |
| -------------------- | -------------------------------------------- | --------- |
| LC00-LC01 基线与契约 | 代码审计、fixture、契约矩阵 PR               | `pending` |
