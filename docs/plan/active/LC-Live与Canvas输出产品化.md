...
| 任务          | 状态      | 交付与验收                                                                                                                                          |
| ------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| LC00 事实冻结 | `PENDING` | 在远端最新 main 上记录 delta、TTS、Artifact、Runtime 当前链路；建立同一 fixture 的延迟基线和失败矩阵；禁止把已实现能力重复建设。                    |
| LC01 契约矩阵 | `PENDING` | 固定 Live 游标、输出意图、Artifact kind/version、Renderer/Runtime 和错误码所有权；列出旧 `canvas`、`note`、`dom_exploration` 的兼容期限和迁移路径。 |
