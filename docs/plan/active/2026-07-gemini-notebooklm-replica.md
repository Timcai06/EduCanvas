...
- [ ] 搜索 Provider Adapter 与 URL 抓取器——webSearch(Tavily)已落地(PR #72)且**实机联网冒烟通过**(2026-07-18:真实浏览器无 mock 驱动,DeepSeek 前导说明→Tavily 检索→带凤凰网/央视来源链接回答当日 WAIC 2026 开幕新闻);URL 抓取器与"结果进统一 Source 管道可勾选引用"仍开放(M3b-C)。
...
- [ ] **PR-U2** 来源常驻面板替代 Sheet 抽屉；行内数字引用跳转原文——前半已交付(PR #67:侧栏来源常驻区,桌面接管资产入口);行内数字引用依赖 M3 synthesis 引用子集,一并交付；
