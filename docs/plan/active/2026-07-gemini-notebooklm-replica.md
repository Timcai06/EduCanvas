...
- Gemini：落地渐变问候；Canvas工具可被用户显式选择，也可被模型建议；Canvas是持久、可跨轮迭代、有版本的共创分栏；
- NotebookLM：一个Notebook整体拥有Sources / Chat / Studio；来源常驻面板包含文档、网页和搜索结果，回答带行内数字引用；Studio生成导图、Slides、测验、闪卡与音频概览。

## 完成画像（计划关闭时用户可以做什么）

以下每条都是可人工验收的用户行为，任何一条做不到就不满足完成条件：

1. 打开站点即统一Notebook界面：侧栏切换整个笔记本（Sources / Chat / Studio一起恢复）；输入框下只保留Canvas工具，选中后本轮产物在右侧分栏展开；
...
- [x] **PR-U3R** Notebook聚合纠偏：将一对一`Space + Conversation`对外统一为Notebook；移除Composer来源芯片与长期来源标签；通用上传固定沉淀到当前Notebook；Studio按Space聚合；Sources / Chat / Studio切换隔离；
- [x] **PR-U4** Canvas 共创化：结构化产物持久挂接Conversation；Canvas修改冻结基线版本和指令进入持久任务，模型结合Notebook对话追加同一Artifact的新版本；历史版本只读恢复，并发与过期修改受保护；
...
flowchart TD
    ...
    U3["PR-U3 Canvas工具"] --> U3R["PR-U3R Notebook聚合纠偏"] --> U4["PR-U4 Canvas 共创"]
    J3 --> U4
...
| 平台计划 P1 | 生产 Turn/Message Parts/Model Run 迁移通用 Operation；Asset / Artifact 归属 Notebook Space | M1 前置或并行                    |
