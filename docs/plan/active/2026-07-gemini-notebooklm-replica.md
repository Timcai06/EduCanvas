...
- [ ] Asset 与 Source/Chunk 合并为单链路（承接旧平台计划缺口 5），含 P0 余项：统一上下文预算策略、通用Conversation的synthesis实际引用子集。K12第一刀已在`feat/20260718-citation-subset`完成：安全回答`[n]`解析、candidate子集/稀疏ordinal持久化、SSE/刷新恢复和行内链接；无合法标记时显式回退全集，多次检索编号统一及通用网页来源链路仍待M3c收口；
...
- [ ] **PR-U2** 来源常驻面板替代 Sheet 抽屉；行内数字引用跳转原文——前半已交付(PR #67:侧栏来源常驻区,桌面接管资产入口);共享Chat渲染器与K12来源徽章跳转已在引用子集第一刀落地，通用网页来源投影与真正的原文定位仍依赖M3c单链路；
