...
| Q01 RAG eval    | `PENDING` | #288 合入后被 #293 revert（延迟基线混入多检索器配置，与 hybrid-only 描述不符）；tooling/evals 已从 main 移除，评测入口不可用；revert PR 声明修正版将重提                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Q05 测试真实性  | `DONE`    | coverage 门禁（telemetry 92/90/83/92、agent-core 96/94/96/97、agent-runtime 78/74/86/80，2026-08-06 基线）+ chromium-mobile 第二环境进默认 lane + hydration 检查 + bundle/route size 基线（jsTotal 4120016B/entry 875476B）+ @ui 独立 lane（chromium+firefox，nightly+路径触发）+ retry/flaky 汇总进 CI Summary（#303，分支 CI 3 次全绿，**待 Code Owner 合并**；门禁合入 main 后生效）                                                                                                                                                                                                                  |
| Q06 供应链发布  | `DONE`    | #304：action 40 位 SHA pin + dependency-review 门禁（high 即 fail）+ 容器 digest pin + migration records 门禁（51 迁移全字段）+ release evidence 语义（passed/skipped/failed 终态语义）+ 供应链文档 08-供应链与发布证据.md；9 项终态全绿（release-evidence SKIPPED 预期，rc1 未发布），**待 Code Owner 合并**                                                                                                                                                                                                                                                                                            |
| Q07 收口        | `DONE`    | docs/06-quality/09-质量基线报告.md（可复现，已验证/未验证/降级三分 + 稳定入口 + 门禁接线清单）+ 本文台账回写；Codex 独立复核后归档                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
