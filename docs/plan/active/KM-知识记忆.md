...
只有前一任务合并并经 Codex 审核通过后，下一任务才解锁。K 子线先建立产品知识主权、
检索作用域与冻结评测，再由 M 子线复用同一 Context Engine 和数据边界实现长期记忆。

| 阶段       | 串行主线                    | 可同时开发的支线       | 阶段出口                         |
| ---------- | --------------------------- | ---------------------- | -------------------------------- |
| K 产品知识 | K00 → K01 → K02 → K03       | C 模型配置线可独立并行 | K03 评测与安全收口通过           |
| M 长期记忆 | M00 → M01 → M02 → M03 → M04 | C 模型配置线可独立并行 | M04 跨会话与 PostgreSQL 验收通过 |

简化依赖图：

```text
K00 ─ K01 ─ K02 ─ K03 ─ M00 ─ M01 ─ M02 ─ M03 ─ M04

C 模型配置线 ────────────────────────────────────────（全程可独立并行）
```

`K03 → M00` 是代码协作门禁，不表示 Product Knowledge 与 Memory 是同一种数据；两者仍使用
不同来源、作用域、版本和失效语义。若必须提前并行，只能在不修改 Context/Data 组合根的
独立文档或夹具范围内进行，并由 Codex 先确认文件边界。
