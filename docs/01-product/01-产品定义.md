...
- **Resource Dock / 资源库**：Dock 恢复当前工作集，Studio 资源库以搜索、筛选、排序和分页管理同一 Notebook 的 Sources 与 Artifacts；
...
Web、TUI 或消息渠道只是访问同一个 Agent 的不同方式。切换 Notebook 必须整体切换 Sources、Conversations、Artifacts、Resource Dock、Studio 资源库、运行记录和相关记忆。
...
Web 的语音是同一对话的两个入口：Dictation 把一次录音识别成可编辑草稿且不自动发送；Live Voice 是当前 Conversation、Turn、消息账本和 Canvas 的沉浸式语音壳，不创建第二份对话或 Agent。Live 内看到的增量回答、工具进度、来源和 Artifact 都来自普通聊天的同一事实，退出后仍留在原消息列。

多模态输入保留原件及其不可变版本，Agent 只消费服务端鉴权并物化的派生表示。用户可选择自动回答、Markdown 文档、类型化交互 Artifact 或隔离 Web App；这些输出继续由唯一 Agent Runtime 和受控 Artifact Tool 产生，不能借输出偏好扩大工具权限。
...
5. **Sources / Artifacts / Resource Dock / Studio / Canvas**：同一 Notebook 下可追溯的资源事实、工作集、资源库与统一工作面；
