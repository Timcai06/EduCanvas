...
- **Studio 是当前 Notebook 的输入/输出工作台**：顶栏 Studio 打开透明浮层，所有导航层级都直接使用 React Bits registry 提供的 OptionWheel，以标签自身的`x/y/rotate`、`curve/tilt`、模糊和透明度形成弯曲滚轮，不绘制圆弧、轨道或圆心。一级提供“添加来源 / 管理来源 / 生成内容 / 查看产物”四个稳定能力域，二级继续以滚轮列出 PDF、图片、网页、真实来源、思维导图、Slides、闪卡、音频和真实产物；只有网页地址等必须输入的最终动作才出现轻量气泡。滚轮改变选择不触发有成本的创建动作，再点中心项或按 Enter 才确认；
