...
- **Studio 是当前 Notebook 的输入/输出工作台**：第一层以顶栏 Studio 按钮为圆心，在浏览器右上角沿四分之一圆径向展开“文件输入 / 内容输出”，单击后才打开大型工作台；第二层弧形轮盘再进入 PDF、图片、网页或思维导图、Slides、闪卡、音频等具体能力。第二层滚轮、拖拽、点击和方向键只改变选择，Enter、空格或再次点击中心项才确认，避免滑动时产生有成本的创建动作；
...
- **辅助层**：Learning Rail / Progress 继续使用模态 Sheet；Studio 初次打开只在右上角呈现斜向弧形轮盘，不挤压对话或历史侧栏。单击“文件输入 / 内容输出”后才展开贴右边缘的非模态工作台，并临时折叠历史侧栏；窄屏时覆盖主内容但不使用 `dialog`、遮罩或焦点陷阱；
