...
- **S2 Chat + Canvas**：Canvas 右侧展开，默认 40:60，中缝可拖拽（对话 28%–62%）、支持全屏与收起；当前可由已有产物卡进入，或先选择输入框下的 Canvas 工具芯片，再从「+」菜单确认生成本轮产物，后台生成完成后自动进入分栏；
...
- Canvas 是一次性的本轮产物意图：选中后，本轮经确认生成的产物在完成时直接进入右侧分栏，提交确认后立即清除选中态；Sources只在当前Notebook的常驻面板管理，勾选仅决定下一轮使用范围，不改变资料归属；
