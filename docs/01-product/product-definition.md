...
Agent 是产品主体，不是某个页面里的功能。Gateway 让同一个 Agent 持续在线并连接不同客户端、消息渠道和设备；Notebook 管理长期上下文；统一 Runtime 负责推理和行动；教育能力提供区别于普通个人 Agent 的默认体验与可信边界。
...
教育不是第二套 Agent Runtime，也不是一个可有可无的演示 Profile。EduCanvas 默认具备：
