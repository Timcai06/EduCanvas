...
```text
用户首次进入语音入口
→ 选择 general / restricted
→ general 需要一次 guardianConfirmed 选择
→ restricted 只允许瞬时云端识别
→ 继续语音提问或回到文本
```

- `general` 的 guardianConfirmed 只是产品体验选择，不是监护关系证明，也不是单独同意记录；
- `restricted` 可用于竞赛演示的临时识别路径，但不应写成音频留存或长期云端转录方案；
- 真人录音、连续 10 分钟、课堂噪声等 live smoke 仍未验收，不能用当前模式选择替代。
