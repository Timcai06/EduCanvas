...
| `00-overview/snapshots` | 已注明日期、停止维护的阶段或研究快照                | 历史核对         |
...
`10-planning/roadmap.md`是跨阶段、相对稳定的路线图；`plan/`是短期执行工作区。Gateway-first 与 Web-first 产品入口已经结档；当前 active 计划只研究第二代架构、能力映射和 ADR 决策，不授权生产架构迁移。两类文档不能互相替代：
