...
`docs/` 是项目的共同事实源：记录产品为什么这样做、系统如何实现、当前决定是什么以及如何验证。聊天记录和口头讨论不能替代这里的文档。

## 从这里开始

| 你的问题 | 推荐阅读路径 |
| --- | --- |
| EduCanvas 是什么、要解决什么问题？ | [产品定义](01-product/01-产品定义.md) |
| 系统如何分层、唯一 Agent Loop 在哪里？ | [系统架构现状](02-architecture/01-系统架构现状.md) → [Agent 编排边界](03-ai/01-智能体编排边界.md) |
| Gateway、模型、数据库分别由谁拥有？ | [Gateway 与多入口](02-architecture/02-网关与多入口.md) → [AI 文档](03-ai/) → [数据设计](04-data/02-数据设计.md) |
| 如何开发、验证和协作？ | [工程文档](05-engineering/) → [团队协作指南](08-collaboration/03-团队协作指南.md) |
| 为什么采用当前技术方案？ | [架构决策 ADR](09-decisions/README.md) |
| 当前在做什么？ | [Active 计划索引](plan/active/README.md) |
...
## Lifecycle and supporting spaces

这些目录支持 canonical 文档，但不构成第二套主题分类。它们的入口文件定义各自的保留与更新规则。

| 目录 | 生命周期职责 | 入口 |
| --- | --- | --- |
| `plan/` | 路线图、active 执行计划与已完成收尾；实现事实必须回写 canonical 文档 | [计划说明](plan/README.md) |
| `research/` | 源码研究、假设、实验与反例；只能形成候选，不替代已接受决定 | [研究说明](research/00-研究说明.md) |
| `archive/` | 停止维护但具有审计或历史核对价值的材料 | [归档说明](archive/README.md) |
| `assets/` | README、文档和设计资产引用的静态媒体；不承载产品或架构事实 | 本目录中的已引用资源 |
...
- [工程逻辑所有权与包映射](05-engineering/05-工程逻辑所有权.md)
