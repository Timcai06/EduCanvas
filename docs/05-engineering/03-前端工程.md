...
- Composer 同时提供 Dictation 与独立 Live SVG 入口：Dictation 只追加可编辑草稿，Live 是当前聊天的全屏壳。Live 直接消费 canonical Assistant delta，并在同一壳内呈现本轮来源、工具真实状态、Artifact 预览和长回答阅读面；退出后焦点和消息列保持连续；
...
当前客户端能力边界必须与后端事实一致：来源面板只显示服务端归属校验后的真实Asset；Turn可以携带结构化Asset引用并在刷新后恢复；General 在视觉 Provider 可用时支持有界原生图片；K1引用来自服务端候选白名单并通过SSE/历史消息呈现；Canvas判分只在可信`ASSESS`状态触发状态推进。Artifact提议、Worker生成、版本、Studio和 Live 内只读预览已经接通。Progress和Studio只能显示服务端已提供的真实数据，不能用Fixture补成“可用”功能。
...
components/                  跨 feature 共享的纯 UI 组件（Sheet、焦点工具、印章等）
features/chat/               两条产品线共享的对话协议与视图
features/canvas/             Artifact运行时
features/voice/              Dictation、Live投影、语义分段、Speech Session与PCM时钟
features/workspace/general/  通用Chat工作区
features/workspace/learning/ K12学习工作区
features/workspace/shared/   工作区专属品牌动效与问候（墨滴/批改笔迹/pixel-blast等）
...
### features 静态边界（ADR-0023）

`features/**` 是浏览器侧 Client 组件层，由 ESLint `no-restricted-imports` 强制以下边界（见 `tests/eslint-boundary.test.ts` 的 negative fixtures）：

- **禁止**导入 `server/**`、`@educanvas/db`、数据库 schema、`server-only`——新违规在 lint 阶段失败；
- **允许** `@/app/actions`（Next server actions）作为受控的跨层入口；
- **feature 间直接导入**（如 `@/features/canvas/canvas-host`）目前以限期 allowlist 收口，限期为 W 线收口前（或最迟下季度），届时收敛到各 feature 公开入口；
- 跨 feature 共享的纯 UI 组件统一放 `components/`，具体 Renderer 不得反向依赖 Workspace。
...
- Live 不维护第二份 Assistant 正文；完整文本来自同一消息 reducer，字幕只消费已排期 PCM cue。代码、长 URL、原始 JSON、Markdown 链接目标和安全字段不得进入朗读段；
