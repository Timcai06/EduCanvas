...
- 已定义`educanvas.operation-continuation.v1`并建立最小化PostgreSQL控制账本：一个Operation可按序保留多个历史等待点、同一时刻仅一个活动等待点，worker以owner + generation + expiry领取；当前尚未接入approval后的原子graphile-worker入队和恢复执行，因此不能把它视为已可用的高风险工具产品能力；
