...
- Tool Effect reconciliation已建立core/runtime/PostgreSQL追加式决议边界和Gateway Internal人工入口：只允许为`outcome_unknown` write Effect记录`confirmed_committed/confirmed_not_committed`，不改写原Effect、Tool Call或Operation终态。自动verifier必须是Adapter专属的只查询实现，并在Effect intention中由服务端冻结身份；运行时和仓储都拒绝调用方换用其他verifier，禁止invoke/replay。MCP v1当前没有可信查询契约，因此自动路径fail closed。人工resolver只能来自Internal token通过后的受信operator/service上下文，学生、模型和请求正文不能自证；
