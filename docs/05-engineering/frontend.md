...
当前客户端能力边界必须与后端事实一致：K1的数据表和防伪引用仓储尚未接入Turn与引用UI；T1的Core/Runtime尚未接入Canvas判分后的Web流程；C1尚无真实Artifact提议、确认、生成和Studio列表。资产抽屉、Progress和Studio只能显示服务端已提供的真实数据或明确的未建设状态，不能用Fixture补成“可用”功能。
