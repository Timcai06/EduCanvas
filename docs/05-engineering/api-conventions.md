...
- `message.citation`：`{type,schemaVersion,turnId,messageId,citationId,marker?,sourceId,documentId,chunkId,label,pageStart,pageEnd}`；`marker`为正文实际出现的`[n]`编号（1..99），旧流缺省时浏览器退化为无编号来源徽章；
