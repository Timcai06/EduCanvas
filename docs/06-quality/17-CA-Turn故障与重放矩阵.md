...
| 工具失败                 | prepare/execute/approval/result 失败或重放                       | 一次 effect；审批与失败语义不互换；terminal recovery 不执行工具                           | `packages/agent-runtime/src/turn-application.tools.test.ts`、`tool-kernel.execution.test.ts`；DB effect ledger integration                                             | `LOCAL_VERIFIED`                                      |
