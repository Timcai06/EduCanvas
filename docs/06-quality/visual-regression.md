...
- Timeline 只改变 `transform` 与 `opacity/autoAlpha`（DrawSVG 笔触只改 stroke-dash），大面积 gradient/blur 装饰层已随 Halo 移除，不再回归；
...
- 首次栅格化后，5 秒录制窗口不应出现装饰动效导致的持续 Layout、连续 Paint 或超过 50ms 的长任务。

当前实现位于 `apps/web/features/workspace/shared/`（`hero-greeting.tsx`、`two-pen-marks.tsx`）与 `apps/web/app/globals.css`。装饰动效只提供「落笔」质感，不承担产品状态、Agent 状态或 Artifact 动画协议。
