...
本文维护可重复的前端行为、无障碍和动效性能证据。仓库不再保存或在
CI 比较操作系统相关的像素快照；稳定规格回写到产品/工程文档，视觉效果由
Code Owner 在真实页面人工验收。自动化继续约束可访问语义、状态、尺寸、
reduced-motion、键盘流程和性能事实，不能用“人工看过”替代这些行为门禁。

持续视觉 Runtime 是可替换的展示 Adapter，不是业务事实源。具体库、Shader、Chunk 大小和帧率属于本质量基线，不构成长期系统架构；任何实现都不得阻塞 Chat、Composer、流式回答、键盘操作或 reduced-motion 路径。

## 自动化与人工边界

| 流程                                     | 自动化证据                                  | 人工证据                    |
| ---------------------------------------- | ------------------------------------------- | --------------------------- |
| Chat-empty 桌面/移动端                   | 标题、Composer、无横向溢出、键盘与移动语义  | 亮/暗主题、层级、排版与动效 |
| AI 不可用诚实失败态                      | 真实错误文案、重试与输入可用性              | 错误态视觉权重              |
| `pipeline_flow` 桌面/移动/reduced-motion | 播放、暂停点、键盘、无溢出、偏好暴露        | 节奏、可读性与性能实测      |
| 主流程、Canvas、判分与进度               | `learning-flow`、`artifact-flow` 等行为 E2E | 分栏比例、密度与响应式细节  |

像素截图不会进入版本控制或作为 PR 必需检查，避免 Darwin/Linux 字体栅格化差异
把视觉决策变成脆弱的二进制维护。普通浏览器 E2E 也不以“不支持 Performance
Observer 时保持为零”的计数冒充性能证据；帧率、Long Task 与 Layout Shift 进入
专用性能采集或真实设备验收。发现布局问题时先增加语义、尺寸、溢出、焦点或状态
断言；只有人工验收负责“是否足够好看”。
...
- [ ] reduced-motion 下无无限呼吸或装饰性自动播放（含输入框边缘扫光整层隐去、只保留静态墨紫聚焦描边）；
- [ ] 输入框边缘墨光扫光平时极淡、聚焦时提亮，且始终在文字与图标之下（不遮挡内容、不拦截输入/焦点）；
- [ ] 一台低端或节能模式设备复核滚动、输入与流式回答并行时无明显掉帧；
- [ ] Canvas Sidecar、Sources Drawer、Studio 等新表面加入后补同视口人工验收。
