...
### Effective subject 投影

Web 对资料身份、认证 session 与数据主体分别记账，不再用“已登录”推断历史数据已迁移：

| 场景                   | profile / session       | effective data owner   | Gateway 主体         |
| ---------------------- | ----------------------- | ---------------------- | -------------------- |
| local，未登录          | none / none             | 配置的 local owner     | 独立 Gateway session |
| local，已有注册 Cookie | registered / registered | 仍为配置的 local owner | 独立 Gateway session |
| 非 local，注册 session | registered / registered | 当前注册账号           | 独立 Gateway session |
| 非 local，匿名 Cookie  | none / none             | 当前浏览器匿名主体     | 独立 Gateway session |

`GET /api/v1/me` 只公开上述分类、数据 scope、Gateway session 独立性和
`automaticOwnershipMigration: false`；不返回 userId、匿名摘要、Cookie、token hash 或 Gateway
凭证。登录与资料显示不会执行账号归属迁移；匿名历史或 local 数据迁移必须另设带审计、回退与用户
确认的任务。
