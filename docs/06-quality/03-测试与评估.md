...
| CI                   | `quality`执行lint、typecheck、unit与coverage门禁；`integration`连接PostgreSQL；普通PR/main push的`e2e`在生产构建上运行8个Chromium核心smoke，Runtime受影响时追加真实composition；`release-evidence`仅在证据包、校验器、Migration/发布治理变更或显式手动发布时运行。`checks`不重复执行测试，只作为唯一required聚合上下文验证本次分类要求的lane全部成功                                                  |
