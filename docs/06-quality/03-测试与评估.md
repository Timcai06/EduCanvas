...
| CI                   | `checks`执行lint、typecheck、unit test与build；`integration`连接PostgreSQL；普通PR/main push的`e2e`在生产构建上运行8个Chromium核心smoke，Runtime受影响时追加真实composition；nightly/手动预发布运行完整稳定矩阵。`checks`仍是唯一required聚合上下文                                                                                                                                     |
...
| 浏览器E2E            | `playwright.pr.config.ts`从完整稳定集显式选择8个`@smoke`，普通PR/main push只在Chromium桌面串行执行；`playwright.config.ts`保留完整稳定定义和chromium/chromium-mobile/firefox矩阵，仅由nightly或手动预发布运行。`@ui`独立lane在PR只跑Chromium，nightly/手动跑Chromium+Firefox；完整集继续覆盖Notebook隔离、Artifact/Worker/版本、权限、SSE、恢复、键盘、性能和安全边界，不以删测试换速度 |
