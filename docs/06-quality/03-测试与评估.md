...
| 浏览器E2E            | 生产构建上46个 test 定义（chromium/chromium-mobile/firefox 三 project 共 138 用例）覆盖Notebook反复切换及Sources/Studio整体隔离、Artifact/Worker/版本/音频、匿名Cookie、无Provider诚实错误、Gateway兼容SSE、Stop/恢复、引用、移动/桌面Canvas、键盘无障碍、Canvas状态矩阵、无重复请求性能证据（W06），跨Chromium/Firefox双内核；视觉回归与沙箱预览；另增 `canvas-resource-access.spec.ts` 覆盖统一资源访问链路（Source/Artifact打开、Notebook隔离、版本选择、旧判分兼容、错误不泄露） |
