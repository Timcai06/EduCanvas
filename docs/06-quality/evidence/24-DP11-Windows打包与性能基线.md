...
- 浏览器登录成功后显式重建经过 Schema 验证的完整 PKCE 授权参数，不再依赖隐式 reload；
- 桌面授权等待增加 10 分钟超时终态，并允许用户随时“重新打开登录”；
- 新增静音、0–100% 音量和动画强度偏好；静音语音 Turn 保留 canonical 文字且不请求 TTS；
- Desktop 56 个测试文件、278 个用例、类型检查和 production build 通过；Web 桌面授权相关
  5 个测试文件、18 个用例通过；
- Windows installer/portable 已基于本次变更重新生成，最小 ASAR 审计通过：15,366,644 字节、
  22 个条目、0 个禁止条目；

| 2026-09-16 产物                  |        字节 | SHA-256                                                            | Authenticode |
| -------------------------------- | ----------: | ------------------------------------------------------------------ | ------------ |
| `EduCanvas-助手-Setup-0.1.0.exe` | 113,983,435 | `230B2AE801B847C5862D326889D6FDAE10E899E0FDD10EEB53E2B1C1A935A776` | `NotSigned`  |
| `EduCanvas-助手-0.1.0.exe`       | 113,775,073 | `969F691A054DCED4A60CFB4B4F3FED088CDCBC95C7C284D67B9E23C8240BE816` | `NotSigned`  |

| 2026-09-16 场景 | 窗口就绪 | 进程 | 隐藏空闲 CPU |    工作集 |  私有内存 |
| --------------- | -------: | ---: | -----------: | --------: | --------: |
| 解包应用        |   736 ms |    4 |        0.14% | 486.7 MiB | 339.8 MiB |
| portable        | 4,231 ms |    5 |        0.19% | 507.1 MiB | 341.4 MiB |

静默首次安装、同版本覆盖和卸载均返回成功；卸载后安装目录与 `educanvas://` 协议注册已清理，
已有 Session 文件保留。安装版的冷启动与已运行单实例协议回跳均只保留一个主进程和一个
“EduCanvas 助手”窗口。交互安装、升级、真实业务流、多 DPI/多屏以及签名分发仍按第五节
保留为人工项，无受信证书时继续保留 `NotSigned` 阻断状态。
