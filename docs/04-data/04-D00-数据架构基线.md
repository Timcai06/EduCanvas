...
| live DB 连接类型          | 本地开发库：docker compose `db` 服务（当次审计容器为 `educanvas-db`，PostgreSQL 16.14，Debian 容器）；仅通过 `docker exec ... psql` 执行**只读 catalog 查询**，未使用写连接                         |
| live DB 身份              | 本地/开发 EduCanvas 数据库（`infrastructure/compose/local.yml` 定义，`config/env/local.env.example` 默认 `postgresql://educanvas:educanvas@127.0.0.1:5434/educanvas`；宿主 5434 映射到容器内 5432） |
