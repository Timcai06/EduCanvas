...
Gateway文本Turn与Web General均写通用Context Snapshot和Model Run；Web General的模型工具还写通用Tool Call，write工具先写`tool_effects`。Gateway创建Operation时生成的`trace_id`原样传给Turn Application；取消先以Actor条件更新`cancel_requested_at`，再由进程信号或Web数据库watcher中止。两条入口都已拆成明确单写：Turn Application只更新assistant消息，Gateway事件循环追加唯一Operation终态；Web Teaching仍待切换对应Adapter。
