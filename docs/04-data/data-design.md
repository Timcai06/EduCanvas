...
当前迁移状态：`model_runs` 与 `turn_context_snapshots` 已 additive 支持 `agent_turn` 和旧教学记录两种互斥形状；通用 Model Run 与 Context Snapshot Ledger 都按 Actor/Operation 重新鉴权。Context Snapshot 还会验证消息属于 Operation 的 Conversation、AssetVersion 属于 Operation 的 Notebook，并拒绝重复ID和同Operation上下文漂移；两类账本分别只保存 Prompt SHA-256 或不可变ID/计数/摘要，不保存正文。`tool_calls` 仍以 `lesson_sessions` 为父实体，教学消息角色仍是 `student/assistant`。旧教学记录暂不回填或物理删除，待教学入口切到统一 Turn Application 并通过保留期验证后再收口。
