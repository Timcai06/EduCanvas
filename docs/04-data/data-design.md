...
`tool_calls` 以可空 `agent_operation_id` 和旧 `session_id / turn_id / teaching_state` 的互斥 CHECK 表达双形状，并继续用全局 `execution_id` 与 `answer_model_run_id + provider_tool_call_id` 两道唯一键阻止重复执行。通用仓储每次读取和状态迁移都重新校验Actor；参数和成功结果最多各接受1000000字节用于生成摘要，数据库只落类型、字节数、元素数和SHA-256。`outcome_unknown` 只允许write调用进入，但它仍是调用审计而非副作用提交证据；M2会以独立effect ledger记录intention与可验证提交。
