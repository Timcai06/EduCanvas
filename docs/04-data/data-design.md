...
当前迁移状态：`model_runs` 已 additive 支持 `agent_turn` 与 `teaching_turn` 两种互斥形状，通用写入由统一 Model Run Ledger Port 按 Actor/Operation 重新鉴权，并且只保存 Prompt SHA-256；`turn_context_snapshots` 与 `tool_calls` 仍以 `lesson_sessions` 为父实体，教学消息角色仍是 `student/assistant`。旧教学 Model Run 暂不回填或物理删除，待教学入口切到统一 Turn Application 并通过保留期验证后再收口。
