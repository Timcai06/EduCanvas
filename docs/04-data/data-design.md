...
- `turn_context_snapshots`：本轮实际选用的历史消息ID、AssetVersion ID、Builder版本和预算计数；`agent_turn` 通过 `agent_operation_id` 绑定通用Operation，旧教学记录继续通过 `session_id + turn_id` 可读；不复制正文；
- `agent_message_parts`：文本、不可变Asset引用和Artifact引用；
- `model_runs`：Provider、模型、Prompt版本/hash、usage、latency和终态；`agent_turn` 通过 `agent_operation_id + conversation_message_id` 关联通用账本，旧 `teaching_turn` 继续通过 `session_id + assistant_message_id` 可读；
...
- `tool_effects`：write工具在调用前写入的effect intention，以及`committed/failed/outcome_unknown`唯一终态；只保存Operation、Tool Call、effect key、语义/回执hash和稳定code；
- `turn_safety_decisions`：输入/输出安全决策的审计投影。

当前迁移状态：`model_runs`、`turn_context_snapshots` 与 `tool_calls` 已 additive 支持 `agent_turn` 和旧教学记录两种互斥形状；三个通用 Ledger 都按 Actor/Operation 重新鉴权。Context Snapshot 还会验证消息属于 Operation 的 Conversation、AssetVersion 属于 Operation 的 Notebook，并拒绝重复ID和同Operation上下文漂移；Model Run 只保存 Prompt SHA-256，Context 与 Tool Call 只保存不可变ID、计数或不可逆摘要，不保存正文。教学消息角色仍是 `student/assistant`。旧教学记录暂不回填或物理删除，待教学入口切到统一 Turn Application 并通过保留期验证后再收口。

Gateway文本Turn已开始写通用Context Snapshot和Model Run。Gateway创建Operation时生成的`trace_id`原样传给Turn Application；取消先以Actor条件更新`cancel_requested_at`，再触发当前进程信号。Gateway入口的消息终态与Operation终态已拆成明确单写：Turn Application只更新assistant消息，Gateway事件循环追加唯一Operation终态；Web旧入口仍使用原子消息/Operation结算，待其迁移时切换对应Adapter。

`model_runs` 迁移保留 PostgreSQL `uuid` 外键，并用可空的 `agent_operation_id / conversation_message_id` 与旧 `session_id / assistant_message_id` 加互斥 CHECK 表达双形状；没有改成无外键的多态 text ID。这样 N-1 教学行无需改写即可读取，新 `agent_turn` 又能由数据库保证 Operation/Message 存在。`operation_kind + operation_id + phase + attempt` 继续作为幂等唯一键，attempt 上限 100 是防止异常重试无限扩张审计行；后续停止旧写入后再以独立迁移收紧非空约束。
