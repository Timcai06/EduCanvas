...
- `research_checkpoints`：只服务Deep Research的有界执行游标，以Operation为唯一身份保存`planning/searching/reading/synthesizing`单调阶段、最多5个规范化已完成查询与15个公开候选URL。它不保存终态、Source或Citation副本；这些事实分别由`agent_operations/gateway_operation_events`、`operation_sources`和`conversation_message_citations`继续唯一负责。浏览器只读取阶段、计数与从权威表派生的序号，不读取查询或URL；
