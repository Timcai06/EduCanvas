...
describe('CI impact classification', () => {
  ...
  it('routes Windows and runtime changes independently', () => {
    const result = classifyChangedPaths([
      'scripts/windows/start-educanvas.ps1',
    ]);
    assert.equal(result.checks, true);
    ...
    const runtimeConfig = classifyChangedPaths([
      'tooling/playwright/playwright.runtime-composition.config.ts',
    ]);
    assert.equal(runtimeConfig.windows, false);
    assert.equal(runtimeConfig.runtime_pressure, true);
    assert.equal(runtimeConfig.e2e, true);
  });

  it('routes grouped local and E2E tooling through their operational lanes', () => {
    const local = classifyChangedPaths([
      'tooling/local/local-orchestrator.mjs',
    ]);
    assert.equal(local.windows, true);
    assert.equal(local.e2e, false);

    const e2e = classifyChangedPaths(['tooling/e2e/e2e-worker-log-audit.mjs']);
    assert.equal(e2e.e2e, true);
  });

  it('routes local environment and Compose configuration to their operational lanes', () => {
    const environment = classifyChangedPaths(['config/env/local.env.example']);
    assert.equal(environment.checks, true);
    assert.equal(environment.windows, true);
    assert.equal(environment.db_integration, false);

    const compose = classifyChangedPaths(['infrastructure/compose/local.yml']);
    assert.equal(compose.checks, true);
    assert.equal(compose.db_integration, true);
    assert.equal(compose.e2e, true);
  });
  ...
  it('treats the PR smoke configuration as an E2E concern without unrelated lanes', () => {
    assert.deepEqual(
      classifyChangedPaths(['tooling/playwright/playwright.pr.config.ts']),
      {
        checks: true,
        db_integration: false,
        worker_integration: false,
...
describe('CI workflow scheduling contract', () => {
  ...
  it('collects independent nightly evidence and publishes one SHA-bound CI artifact', () => {
    ...
    const generator = readFileSync(
      new URL('./ci-evidence.mjs', import.meta.url),
      'utf8',
    );
