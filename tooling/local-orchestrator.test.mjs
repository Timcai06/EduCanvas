  ...
  const child = spawn(
    ...
    ['tooling/local-orchestrator.mjs', 'all'],
    {
      ...
      env: {
        ...
        FAKE_PS_MAP: `${fixture.pid}:node ${repoPath}/tooling/local-core-cleanup.fixture.mjs;${worker.pid}:pnpm --filter @educanvas/worker dev`,
      },
      stdio: ['ignore', 'pipe', 'pipe'],
    },
  );
...
test('recorded PID 被无关进程复用 → 清理跳过而非误杀（fail closed）', async () => {
  ...
  try {
    result = await run(['all'], {
      ...
      // 特征（PID 复用场景）→ 必须跳过。
      FAKE_LSOF_MAP: `${gatewayPort}:${fixture.pid}`,
      FAKE_PS_MAP: `${fixture.pid}:node ${repoPath}/tooling/local-core-cleanup.fixture.mjs;${worker.pid}:node -e inline http server`,
    });
