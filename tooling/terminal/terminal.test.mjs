...
import { TOKENS, RESET, DEPTHS, ansiCode, paint } from './theme.mjs';
...
import { hyperlink } from './links.mjs';
import { SPINNER_FRAMES, spinnerFrame } from './progress.mjs';
...
test('links: OSC8 只在显式启用且 http/https 时输出', () => {
  assert.equal(
    hyperlink('web', 'http://127.0.0.1:3000', { enabled: true }),
    '\x1b]8;;http://127.0.0.1:3000\x1b\\web\x1b]8;;\x1b\\',
  );
