...
describe('Q01 RAG 冻结评测 v1', () => {
  ...
  it('生成冻结评测报告并验证回退诚实标记', async () => {
    ...
    const report: {
      ...
      retrievers: {},
      perQuery: [],
      latencyMs: { p50: undefined, p95: undefined, samples: 0 },
    ...
    console.log(
      `延迟 p50=${report.latencyMs.p50?.toFixed(1)}ms p95=${report.latencyMs.p95?.toFixed(1)}ms (n=${report.latencyMs.samples})`,
    );
    console.log(
