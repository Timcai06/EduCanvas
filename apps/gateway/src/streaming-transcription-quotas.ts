 ...
 * - 数值以"交互式语音片段 + 单机 Gateway"为基线取保守值，理由见各常量
 *   注释；多实例/多用户部署的调优方向见 docs/plan/active/UV-画布语音.md
 *   V13 一节。
 *
 * ## 基线计算
 *
 * 音频速率冻结为 16 kHz × 1 ch × 2 字节 = 32 000 B/s（V04 常量）。单轮
 * 用户语音通常为秒级，Live Voice 会话可持续数分钟；连接与 Provider
 * Session 都必须有独立上限，避免单用户或上游阻塞耗尽 Gateway 资源。
 */
...
/**
 * 默认配额。以交互式语音基线取保守值；未来调优应改这里 + 工程文档，而不是在
 * 运行时放宽（见 README 与计划文档）。
 */
export const STREAMING_TRANSCRIPTION_DEFAULT_QUOTAS: StreamingTranscriptionQuotas =
  {
    // 2：正常情况下只有一条 Live Voice 连接；2 已覆盖"一条在用 + 一条
    // 重连窗口"，超过该值通常意味着重复标签页或客户端生命周期异常。
    ...
    // 限；单机 32 条并发连接已保守，防不同用户各开多连接耗尽
    // socket 描述符与事件循环。
    maxConnectionsGlobal: 32,
    // 8：每个活跃 Session 都占用 Provider 连接、事件监听器和 Gateway 缓冲；
    // 8 个并发作为单机保守默认值，超出拒绝。这是 Session 槽上限：终态
    // 形成即释放，与浏览器连接是否关闭解耦（REVISE）。
    maxActiveSessionsGlobal: 8,
    // 10 分钟：Live Voice 单次连接通常只持续数分钟；超过视为异常占用（含不发
    ...
    // 60 秒 × 32 000 B/s = 1 920 000：单轮语音通常为秒级，60 秒累计音频
