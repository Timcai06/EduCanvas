...
import { AvatarUploadError, detectAvatarImage } from '@/server/profile/avatar';
...
export async function GET(): Promise<Response> {
  ...
  if (!avatar) return jsonError(404, 'avatar_not_found');
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!(file instanceof File)) {
      return jsonError(400, 'invalid_avatar');
    }
    ...
    if (error instanceof BoundedMultipartError) {
      return error.code === 'multipart_too_large'
        ? jsonError(413, 'avatar_too_large')
        : jsonError(415, 'invalid_avatar');
    }
    ...
    return jsonError(503, 'avatar_unavailable');
  }
}
