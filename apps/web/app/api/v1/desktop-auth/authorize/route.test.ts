...
vi.mock('@/server/identity/anonymous-identity', () => ({
  readAnonymousIdentity: mocks.readRuntimeIdentity,
}));
...
describe('POST /api/v1/desktop-auth/authorize', () => {
  beforeEach(() => {
    ...
    mocks.readRuntimeIdentity.mockResolvedValue({
      token: '',
      studentId: 'user:one',
    });
  ...
  it('binds the same runtime identity used by Web in local deployment', async () => {
    mocks.readRuntimeIdentity.mockResolvedValueOnce({
      token: '',
    ...
    expect(mocks.issueAuthorizationCode).toHaveBeenCalledWith(
      expect.objectContaining({ userId: 'local:owner' }),
    );
  });
});
