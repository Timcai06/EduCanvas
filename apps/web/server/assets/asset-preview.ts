...
export async function loadOwnedAssetPreviewDetail(input: {
  ...
  if (version.mimeType === 'application/pdf') {
    /* ADR-0026 决定 2/6：MinerU 结构化派生可用时提供结构化阅读视图。
       前端默认仍渲染原 PDF（pdf.js），通过显式切换进入阅读视图；
       degraded 降级文本同样投影，由前端标注 provenance 区分（不冒充 structured）。 */
  ...
  if (
    ...
    /* ADR-0026 决定 2/6：MinerU 结构化派生可用时优先提供结构化阅读视图，
       ...
       structured 与 degraded 都投影派生文本（与 PDF 分支同构，降级由前端
       标注 provenance 区分）；MinerU 结果损坏时 resolveStructuredMarkdown
       返回 null，自动回退 mammoth 预览。 */
    const representation = version.textRepresentation;
    const derivedMarkdown =
      representation &&
      representation.status === 'ready' &&
    ...
    if (!derivedMarkdown || representation?.quality === 'degraded_plain_text') {
      /* 处理中/失败/无表示或降级时保持 mammoth 原格式近似预览。 */
