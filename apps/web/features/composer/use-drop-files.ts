...
export function useDropFiles(maxFiles = 8, maxBytes = 10 * 1024 * 1024) {
  ...
  const addFiles = useCallback(
    (incoming: FileList | File[]) => {
      ...
      const allowed = array.filter((f) => {
        if (f.size > maxBytes) { setRejected(`"${f.name}" 超过 ${Math.round(maxBytes / 1048576)}MB 限制`); return false; }
        return true;
      });
      setFiles((prev) => {
        ...
        if (allowed.length > remaining) { setRejected(`最多添加 ${maxFiles} 个文件`); }
        const added: PendingFile[] = toAdd.map((file) => {
          idCounter.current += 1;
          const isImage = file.type.startsWith('image/');
          return { id: `pending-${idCounter.current}`, file, previewUrl: isImage ? URL.createObjectURL(file) : null };
        });
        return [...prev, ...added];
      });
    },
  ...
  const clearFiles = useCallback(() => {
    setFiles((prev) => { for (const f of prev) revokeFile(f); return []; });
    setRejected(null);
  }, [revokeFile]);
