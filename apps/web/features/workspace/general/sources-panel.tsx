...
import { deleteAsset, importLinkAsset } from '@/features/assets/asset-client';
import {
  FileArrowUp,
  FilePdf,
  Image as ImageIcon,
            ...
            >
              ...
              {onDelete ? (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    void deleteAsset(asset.id).then(() => onDelete(asset.id));
                  }}
                  className="grid size-5 shrink-0 place-items-center rounded-full text-ink-faint transition-colors hover:bg-bad-soft hover:text-bad"
                >
