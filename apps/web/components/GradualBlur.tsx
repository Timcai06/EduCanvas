...
const PRESETS: Record<
  NonNullable<GradualBlurProps['preset']>,
...
const CURVE_FUNCTIONS: Record<
  NonNullable<GradualBlurProps['curve']>,
...
const getGradientDirection = (
  ...
  const directions: Record<
    NonNullable<GradualBlurProps['position']>,
    string
  > = {
    top: 'to top',
    bottom: 'to bottom',
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const mobile =
        key === 'height' ? config.mobileHeight : config.mobileWidth;
      const tablet =
        key === 'height' ? config.tabletHeight : config.tabletWidth;
      const desktop =
        key === 'height' ? config.desktopHeight : config.desktopWidth;
      if (viewportWidth <= 480) setValue(mobile || baseValue);
      else if (viewportWidth <= 768) setValue(tablet || baseValue);
      else if (viewportWidth <= 1024) setValue(desktop || baseValue);
      else setValue(baseValue);
    };
