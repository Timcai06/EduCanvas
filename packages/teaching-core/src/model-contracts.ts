...
/** 业务任务别名。供应商模型 ID 不得作为任务别名进入领域代码。 */
...
/** 正常教学 Turn 只能流式调用；结构化调用仅服务于非 Turn 任务。 */
export const structuredTaskAliases = [
  'artifact.generate',
  'retrieval.query_rewrite',
] as const satisfies readonly Exclude<TaskAlias, 'teaching.turn'>[];
...
/** 正常教学 Turn 的两种模型运行阶段；一轮不得出现第三个阶段。 */
...
/** 适配器可见的受控工具定义；handler 与供应商 SDK 类型都不会进入契约。 */
...
/**
 * 工具执行后回注 synthesis 的完整、已验证交换。
 * `arguments` 不能依赖 Gateway 内存，适配器需据此重建 assistant.tool_calls。
 */
...
/** DOM/Node AbortSignal 都满足的最小跨运行时取消契约。 */
...
/** 单次流式模型运行请求。正常教学 Turn 的 taskAlias 固定为 teaching.turn。 */
export interface StreamTurnTextRequest {
...
/** Token 统计统一为累计值；供应商不提供的字段由适配器归零。 */
...
/** 一次供应商调用完成后可持久化的、无 Prompt 正文的审计元数据。 */
...
/**
 * 供应商适配器用于显式上报已归一化错误的异常类型。
 * `cause` 仅供服务端内部观测，不能序列化到客户端事件。
 */
...
/**
 * `tool_call` 使用 JSON 参数分片，避免供应商的增量事件泄漏到 core。
 * 同一 callId 的分片必须按序到达，`done=true` 的分片结束并触发 JSON 解析。
 */
...
/** 供应商无关的流式模型事件。每次调用必须且只能以 completed 或 failed 结束。 */
