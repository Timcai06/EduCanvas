...
const piiPatterns: readonly RegExp[] = [
  ...
  /(?:^|\D)(?:\+?86[ -]?)?1[3-9]\d{9}(?:\D|$)/,
  /\b\d{17}[\dx]\b/i,
  /(?:手机号|手机号码|联系电话|phone|mobile|call me at)[^\d+]{0,12}\+?\d[\d ()-]{7,}\d/i,
  /(?:身份证(?:号|号码)?|identity number|id number|id no\.?)[^\d]{0,8}\d{6,18}[\dx]?/i,
  /(?:我的真实姓名是|真实姓名[:：]|real name is)\s*[\p{L}·.' -]{2,40}/iu,
  /(?:我家住在|我的住址是|家庭住址|home address is|i live at)\s*[:：]?\s*.{4,}/iu,
  /(?:密码|口令|验证码|password|passcode|verification code)\s*(?:是|为|[:：=])\s*\S{4,}/iu,
];
...
/**
 * Provider 前的纯输入评估。结果不包含输入、命中片段、规则或可逆摘要。
 */
