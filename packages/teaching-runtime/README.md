...
- `src/tool-executor.ts`：只为旧契约与迁移回滚保留的`TeachingToolExecutor`，生产入口禁止构造；
- `src/tool-kernel-adapter.ts`：把Teaching Tool适配到统一`ToolKernel`；教学Profile上下文必须先通过Schema，状态白名单由Kernel的Profile能力集表达；
- `src/turn-orchestrator.ts`：提供生产使用的纯`createTeachingTurnPromptMessages()`材料构建器；遗留`TeachingTurnOrchestrator`只为兼容测试保留，生产两阶段循环由`agent-runtime`拥有；
...
- `TeachingToolExecutor`和`TeachingTurnOrchestrator`仍为回滚/旧契约测试保留，M5在无调用与回滚证据齐全后删除。
