...
- `src/tool-kernel-adapter.ts`：把现有Teaching Tool适配到统一`ToolKernel`；教学Profile上下文必须先通过Schema，状态白名单由Kernel的Profile能力集表达；
...
- Teaching兼容Adapter已有契约测试，但Web生产组合根尚未切换到统一Tool Kernel；迁移完成前`TeachingToolExecutor`仍保留。
