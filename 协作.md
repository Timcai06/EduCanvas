...
分支名格式：`类型/做什么`，完整规则和六种类型见下方 **[命名与提交规范速查表](#命名与提交规范速查表)**。
...
提交说明的格式是 `类型: 做了什么`，一次提交只做一件事。类型清单和好坏示例见下方 **[命名与提交规范速查表](#命名与提交规范速查表)**。
...
|---|---|---|---|
| `feat` | 新功能 | `feat/lesson-canvas` | `feat: add lesson canvas` |
| `fix` | 修 bug | `fix/chat-stream-timeout` | `fix: prevent quiz double submit` |
| `docs` | 只改文档 | `docs/update-roadmap` | `docs: update roadmap for phase 2` |
| `refactor` | 重构（行为不变） | `refactor/canvas-events` | `refactor: extract event emitter` |
| `test` | 增改测试 | `test/quiz-grading` | `test: cover quiz grading edge cases` |
| `chore` | 依赖、配置、杂项 | `chore/upgrade-gsap` | `chore: upgrade gsap to 3.14` |

### 三条格式规则

1. **分支名**：`类型/做什么`，全小写英文加短横线，不写自己的名字和日期（分支合并就删，名字留不下来）；
