...
分支名统一为 **`类型/YYYYMMDD-简短任务名`**：全小写英文，日期使用创建分支当天的中国时区日期。例如今天创建 Canvas 任务就写 `feat/20260713-lesson-canvas`。日期只标记这项工作的开始，不要因后续提交跨天而改分支名；一个新的独立任务就新建一个分支。

前缀表示类型——

- `feat/YYYYMMDD-xxx` 新功能，如 `feat/20260713-lesson-canvas`
- `fix/YYYYMMDD-xxx` 修 bug，如 `fix/20260713-chat-stream-timeout`
- `docs/YYYYMMDD-xxx` 改文档，如 `docs/20260713-backend-architecture`
- `refactor/YYYYMMDD-xxx` 重构，如 `refactor/20260713-extract-canvas-registry`
- `test/YYYYMMDD-xxx` 测试，如 `test/20260713-artifact-validation`
- `chore/YYYYMMDD-xxx` 配置或杂项，如 `chore/20260713-upgrade-pnpm`
...
提交说明的格式仍是 `类型: 做了什么`，**不加日期**。日期用于分支定位，提交说明和 PR 标题会进入 main 的长期历史，应该优先表达改了什么。类型有：`feat`（新功能）、`fix`（修复）、`docs`（文档）、`refactor`（重构）、`test`（测试）、`chore`（配置杂项）。一次提交只做一件事。**不要写 "update"、"修改一下" 这种看不出内容的说明。**
