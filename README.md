...
| 已落地 | 部分落地 | 尚未完成 |
| --- | --- | --- |
| 通用 Chat 默认入口；K12 为显式 Vertical Agent | 通用 Turn 幂等、持久化与刷新恢复 | 通用 Turn 取消与后台恢复 |
...
3. **P2 Agent Runtime（下一主阶段）**：补齐通用 Turn 取消、Model Run/Trace、Tool Registry、Policy 与 Agent Profile；
