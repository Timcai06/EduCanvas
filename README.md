...
| Web、可运行 TUI、Telegram 私聊适配器                      | Telegram 只有离线官方协议 Fixture，未做 live | 群聊、语音渠道及真实 Telegram 凭据验证     |
...
- Gateway：<http://127.0.0.1:3200>（公共Client/Node仅在配置三项Gateway密钥后开启）
...
Gateway启用后，按需单独使用其他入口：

```bash
# 以下命令从本地.env读取Gateway/数据库/Telegram配置
set -a; . ./.env; set +a

# TUI：先登录，再列出会话/聊天/恢复/审批
pnpm --filter @educanvas/tui exec tsx src/index.ts login http://127.0.0.1:3200 <user-id>
pnpm --filter @educanvas/tui exec tsx src/index.ts conversations
pnpm --filter @educanvas/tui exec tsx src/index.ts chat <conversation-id> "解释光合作用"

# Telegram：先由管理员绑定，再单独启动长轮询Adapter
pnpm --filter @educanvas/telegram exec tsx src/index.ts bind <user-id> <telegram-user-id> <conversation-id>
pnpm --filter @educanvas/telegram dev

# Capability Node：配对后单独运行；只读根由EDUCANVAS_NODE_READ_ROOTS_JSON配置
pnpm --filter @educanvas/node exec tsx src/index.ts pair http://127.0.0.1:3200 <user-id>
pnpm --filter @educanvas/node dev
```

bootstrap token是管理员/本地建联密钥，不是最终用户密码；不要放入命令历史或分发给学生。生产登录和渠道自助绑定仍需正式IdP。
