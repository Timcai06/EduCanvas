...
以教育能力为核心的通用个人 Agent 平台（浙江省大学生人工智能竞赛 JBGS-2026-02：多模态 K12 人工智能通识课教学助手对话智能体）。EduCanvas 的第一身份是通用个人 Agent，教育是核心能力域，不是第二套 Agent。所有产品、架构、AI、数据设计文档在 `docs/`，入口是 `docs/README.md`；面向人的协作指南是 `docs/08-collaboration/03-团队协作指南.md`。
...
API Key、密码、Token、私钥、`.env` 真实配置、学生个人信息、模型权重、构建产物、临时缓存、未确认授权的教材/图片/视频。环境变量模板放 `config/env/local.env.example`。

## 文档同步义务

- 改动影响产品行为、接口或数据结构时，同一个 PR 里更新 `docs/` 对应文档
- 重大技术选型变化，先在 `docs/09-decisions/` 新增 ADR（模板在 `docs/templates/`）
...
- 注释只写代码本身表达不了的“为什么”：设计意图、约束原因、失败风险和对应的`docs/`文档或ADR；禁止复述函数名、语句或循环过程
...
- 教学流程由五态确定性状态机约束（DIAGNOSE→EXPLAIN→DEMONSTRATE→PRACTICE→ASSESS）；REMEDIATE/ADVANCE只是ASSESS出口决策，模型只负责表达和受控工具调用
- Embedding 必须记录模型、版本、维度、指令和切块版本
- LangChain 不作为核心依赖；领域状态保存在自己的数据库，不放在 Agent 框架内部
- 核心纯逻辑（Canvas 协议校验、教学状态机、掌握度计算）必须在实现它们的同一 PR 内附带单元测试；UI 组件不作此强制
